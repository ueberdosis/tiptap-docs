  ...
  ({ children, className, label, ...rest }, ref) => {
    return (
      <div
        {...rest}
        className={cn(
          'flex items-center justify-between gap-4 text-foreground mb-2 uppercase text-label font-bold pl-2.5',
          className,
        )}
        ref={ref}
      >
  ...
  ({ asChild, children, className, isActive, ...rest }, ref) => {
    ...
    return (
      <Component
        {...rest}
        className={cn(
          'flex items-center justify-between w-full px-2.5 py-1.5 text-sm text-foreground rounded-md',
          !isActive
            ? 'bg-transparent hover:text-foreground hover:bg-sidebar-hover'
            : 'bg-sidebar-active text-selection-accent font-semibold',
          className,
        )}
        ref={ref}
      >
