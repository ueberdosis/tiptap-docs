...
function Card({ href, title, isNext }: { href: string; title: string; isNext?: boolean }) {
  return (
    <Link
      href={href}
      className="bg-card text-card-foreground group flex gap-6 p-6 items-center border border-border rounded-lg shadow-sm hover:shadow-md transition"
    >
      {!isNext ? <ArrowLeftIcon className="size-5 flex-none" /> : null}
      <span className="flex flex-col gap-2 flex-1">
        <span className="text-sm text-foreground-subtle">{isNext ? 'Next up' : 'Previously'}</span>
