...
export const PageHeaderBreadcrumbs = forwardRef<HTMLDivElement, PageHeaderBreadcrumbsProps>(
  ({ asChild, config, className, ...props }, ref) => {
    ...
    const breadcrumbClass = cn(
      'text-sm leading-[110%] text-foreground-subtle font-semibold select-none',
    )

    const clickableBreadcrumbClass = cn(breadcrumbClass, 'text-selection-accent hover:underline')
        ...
        <Component className={wrapperClass} {...props} ref={ref}>
          {crumbs.map((crumb, i) => (
            <div key={`${crumb.href ?? crumb.text}-${i}`}>
              ...
              {i < crumbs.length - 1 ? (
                <span className="ml-2 text-sm font-bold select-none text-foreground-subtle">/</span>
              ) : null}
            </div>
          ))}
        </Component>
