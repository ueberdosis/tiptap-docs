...
`Editor` comes from `@tiptap/core`; `Schema` comes from `@tiptap/pm/model`. See [shared types](/compose/slots/api-reference/types).
...
Creates a policy allowing users to fill existing Slots while protecting the surrounding document and field definitions. Requires [Content Protection](/content-protection/getting-started/overview) to enforce it.
...
See the [policy language](/content-protection/api-reference/policy) for precedence and selector behavior. Validation functions are documented separately under [Validation](/compose/slots/api-reference/validation).
