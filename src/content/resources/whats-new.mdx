...
<Callout title="Beta Support" variant="hint">
  Tiptap V3 is currently in Beta, with a stable release scheduled for later in 2025. Please support
  the open source release by submitting issues in the [GitHub
  repository](https://github.com/ueberdosis/tiptap/tree/next).
</Callout>
...
## Updating beta packages

When updating any of the Tiptap or Tiptap Pro beta packages, please ensure that all related packages are upgraded simultaneously using the `@next` tag. This guarantees that you are installing the latest beta versions across the board. Partial updates may lead to peer dependency conflicts, as we currently enforce strict version alignment across all Tiptap packages during the beta phase.
...
| **New `shouldRerenderOnTransaction` option with `false` default**  | The new `shouldRerenderOnTransaction` option is disabled by default to minimize rerendering with React components. This can cause your editor component to not rerender as expected. Read more [here](/examples/advanced/react-performance).                     |
