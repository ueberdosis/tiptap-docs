...
Biome ensures a consistent code style. To check for errors, run `$ pnpm run lint`. To apply automatic fixes, run `$ pnpm run lint:fix`. Make sure the checks are passing before sending a pull request.
