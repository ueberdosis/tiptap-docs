...
Every import returns a `logs` object next to the content, holding `warn` and `error` arrays. A
warning means content did not make it into the document, which is the first thing to check when a
document arrives with something missing.
