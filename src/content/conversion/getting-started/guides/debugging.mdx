...
meta:
  ...
  description: How to read conversion warnings, inspect intermediate JSON, and diagnose why imported content isn't rendering in your editor.
  category: Conversion
---
...
Every import returns a `logs` object next to the content, holding `warn` and `error` arrays. There is
nothing to switch on. A warning means content did not make it into the document, which is the first
thing to check when a document arrives with something missing.
...
The logs reach the `onImport` callback on `context`, so you can log them or show them to whoever
uploaded the file:
...
editor
  ...
  .importDocx({
    file,
    onImport(context) {
      for (const warning of context.logs.warn) {
...
<Callout title="`verbose` was removed" variant="info">
  Logs used to sit behind a `verbose` bitmask that recorded nothing by default, so an import that
  quietly dropped every image looked identical to a clean one. Warnings and errors now always come
  back. Requests that still send `verbose` are accepted and the value is ignored.
</Callout>
...
| ------ | ------------------------------------------------------------------------------------------------------------------------------------ | ---------------------------------------------------------------------------------------------------------------------------------------------------- |
| `401`  | Missing or invalid JWT.                                                                                                              | Regenerate the token server-side.                                                                                                                    |
...
- **The DOCX didn't actually carry headers and footers.** Word can store templates and not commit them. Check the logs (above) to see what the converter extracted.
