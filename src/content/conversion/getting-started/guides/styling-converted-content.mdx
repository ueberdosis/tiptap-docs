...
For a quick overview of which features are supported at each stage, see the [Supported features](/conversion/getting-started/feature-support-matrix) matrix. For per-feature details including what attributes are extracted and what extensions are needed, see the [Content reference](/conversion/content-types/overview) pages.
...
| Letter spacing | `textStyle.letterSpacing` | [Requires extending TextStyle](#extending-extensions-for-unsupported-attributes) | `style="letter-spacing: 2px"` |
...
If you have these extensions installed (see [ConvertKit](/conversion/import/docx/convertkit) for the recommended set), inline formatting renders automatically with no additional CSS required.
...
| Styling | Node attribute | Format | With standard extensions | With [ConvertKit](/conversion/import/docx/convertkit) |
| --- | --- | --- | --- | --- |
...
| Right indent | `indentRight` | Number (pixels) | Not rendered | `padding-right: Npx` |
| First line indent | `firstLineIndent` | Number (pixels) | Not rendered | `text-indent: Npx` (a negative value is a hanging indent) |
...
| Keep with next | `keepNext` | Boolean | Not rendered | Kept in the JSON for export, no CSS output |
| Keep lines together | `keepLines` | Boolean | Not rendered | Kept in the JSON for export, no CSS output |

`indent` and `firstLineIndent` are read differently on a paragraph inside a list item. When the list names a numbering format registered with the editor, the two position the whole list item, marker and text together, rather than padding the paragraph inside it. A list with no registered format renders them as `padding-left` and `text-indent` on the paragraph, as the table describes. See [Indents on a single item](/conversion/export/docx/ordered-list-numbering#indents-on-a-single-item).

<Callout variant="warning" title="Schema validation strips unrecognized attributes">
  When you call `setEditorContent()`, ProseMirror validates the content against your editor's schema. Attributes not defined in the node spec are removed. The raw JSON in `context.content` still contains all attributes, but they are lost once the content is loaded into the editor. To preserve them, install [ConvertKit](/conversion/import/docx/convertkit) (which extends the schema for all the attributes above) or extend the node schema yourself.
</Callout>
...
In the browser, the export reads the CSS your editor renders with and turns it into the DOCX styles. The exported file matches what the user saw while editing, including the CSS from the sections above, without extra configuration. See [Document styles](/conversion/export/docx/document-styles).

Use `styleOverrides` when the file should look different from the editor, for example to follow a corporate template. Your overrides apply over the styles the export read:
    ...
    paragraphStyles: [
      {
        ...
        paragraph: {
          // 10pt after, 1.15 line height. `lineRule: 'auto'` makes `line` a multiple of
          // the font's line height, instead of the minimum height the export read.
...
The export can't read your CSS on a server, with `documentStyles: false`, or with a headless editor. It then starts from the package's [default styles](/conversion/export/docx/styles#tiptaps-export-default-styles), with Arial 11pt for Normal. To match your editor there, set `styleOverrides` like above.

## Extending extensions for unsupported attributes

[ConvertKit](/conversion/import/docx/convertkit) already extends `Paragraph`, `Heading`, `Image`, and the table stack with the DOCX-specific attributes the Convert API produces. If you use ConvertKit, the paragraph and heading attributes in the table above render automatically; you do not need to write the extension yourself.
...
The same pattern works for any other attribute the converter emits but no extension renders. See the [custom extensions guide](/editor/extensions/custom-extensions/extend-existing) for the full mechanics of extending existing extensions.
