...
In the browser, the DOCX styles follow the CSS your editor renders with. The exported file matches what the user saw while editing, including the CSS from the sections above, without extra configuration. See [Document styles](/conversion/export/docx/document-styles).

Use `styleOverrides` when the file should look different from the editor, for example to follow a corporate template. Your overrides apply on top of your CSS:
    ...
    paragraphStyles: [
      {
        ...
        paragraph: {
          ...
          // the font's line height, instead of the minimum height from your CSS.
...
The export can't use your CSS on a server, with `documentStyles: false`, or with a headless editor. It then starts from the package's [default styles](/conversion/export/docx/styles#tiptaps-export-default-styles), with Arial 11pt for Normal. To match your editor there, set `styleOverrides` like above.
