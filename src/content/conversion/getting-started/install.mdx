...
Send the token in the `Authorization: Bearer <jwt>` header for REST calls, or pass it through the `token` option on the editor extensions. The token is all a request needs.
...
<Callout title="DOCX export does not require authentication" variant="info">
  The `@tiptap-pro/extension-export-docx` package handles conversion entirely on the client. No
  token or server calls needed.
</Callout>
