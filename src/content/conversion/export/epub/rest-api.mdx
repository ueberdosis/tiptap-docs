...
| `placeholders`       | `Object \| false` | Rename map (`{ page?, total? }`) for page-number tokens in plain-text headers/footers, matching `Pages.configure({ placeholders })`. Pass `false` to disable substitution. See [Custom token names](/conversion/export/docx/rest-api#custom-token-names-placeholders). | `undefined` |
