...
| `headerFooterOverrides` | `Object` | Per-field overrides scoped to header/footer content (same five keys as the element overrides). See [Element Overrides](#element-overrides). | `undefined` |
| `placeholders`       | `Object \| false` | Rename map (`{ page?, total? }`) for page-number tokens in headers/footers of any slot shape (plain text or stringified Tiptap JSON), matching `Pages.configure({ placeholders })`. Pass `false` to disable substitution. See [Custom token names](/conversion/export/docx/rest-api#custom-token-names-placeholders). | `undefined` |
| `customNodeDsl` | `Object` | [Custom-node DSL](/conversion/export/docx/custom-nodes-dsl) rules for rendering custom Tiptap nodes into the exported file. | `undefined` |
...
<Callout title="Plain text vs. Tiptap JSONContent">
    Each header value can be either a **plain text string** (produces a simple unstyled header) or a **stringified Tiptap JSONContent** object (enables rich formatting such as bold, italic, links, etc.). When passing JSONContent, stringify the object with `JSON.stringify()` before sending it in the request body. In both shapes, `{page}`, `{total}`, and the `{numpages}` synonym render as live page-number fields; rename or disable them with the top-level `placeholders` property.
</Callout>
...
<Callout title="Plain text vs. Tiptap JSONContent">
    Each footer value can be either a **plain text string** (produces a simple unstyled footer) or a **stringified Tiptap JSONContent** object (enables rich formatting such as bold, italic, links, etc.). When passing JSONContent, stringify the object with `JSON.stringify()` before sending it in the request body. In both shapes, `{page}`, `{total}`, and the `{numpages}` synonym render as live page-number fields; rename or disable them with the top-level `placeholders` property.
</Callout>
