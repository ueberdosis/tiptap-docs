...
Every export starts from the styles below. In the browser, the export swaps most of them for styles it reads from your editor's CSS, see [Document styles](/conversion/export/docx/document-styles). You get them unchanged on a server, in a headless or hidden editor, or when you set `documentStyles: false`. Your `styleOverrides` apply on top either way.

```ts
{
  // The docx library writes these styles itself, so the export changes them through
  // the `default` hooks. Word keeps the first definition of a style id.
  default: {
    // Document defaults, which every style builds on
    document: {
      run: {
        font: 'Arial',
  ...
  characterStyles: [
    // Code style (used for inline code)
    {
      id: 'Code',
      name: 'Code',
...
Links use the docx library's own `Hyperlink` character style, which is `0563C1` with a single underline. `Heading 6` also keeps the docx library's definition, colored `1F4D78`.

## Element overrides

Element overrides are separate from `styleOverrides`. While `styleOverrides` defines **named paragraph styles** (Heading 1, Normal, etc.) that Word applies through its style system, **element overrides** apply base defaults directly to every paragraph, text run, table, table row, table cell, or image during the conversion process.

This gives you fine-grained control over the raw docx properties of every element without needing to define named styles.

In the browser, the export also builds element overrides from your editor's CSS, for example for the text in table cells. Yours apply on top, and each top-level key you set replaces the one the export read. See [Document styles](/conversion/export/docx/document-styles#how-your-options-combine-with-the-reading).
...
Accepts any `ITableRowOptions` property except `children`. These values are spread **first** as base defaults, before each row's cells are added. Useful for row-level layout that has no per-style equivalent, such as keeping a row together across a page break (`cantSplit`), repeating a row as a header on every page (`tableHeader`), or pinning a fixed row `height`.
...
Element overrides apply to header and footer content as well as the body: a `paragraphOverrides`, `textRunOverrides`, `tableOverrides`, `tableRowOverrides`, `tableCellOverrides`, or `imageOverrides` value set at the top level flows into the running header and footer too.
...
> **Scope**: `headerFooterOverrides` only applies to header/footer content auto-extracted from the [Pages extension](/pages/getting-started/overview). If you build headers/footers manually with `Docx.Header` / `Docx.Footer` instances or with custom factories passed to the `headers` / `footers` options, those constructions bypass `headerFooterOverrides`, so apply the overrides yourself inside your factories.
