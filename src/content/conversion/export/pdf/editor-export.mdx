 ...
 * @example editor.commands.exportPdf({ onExport: (context) => {} })
 */
exportPdf: (options?: { onExport?: (context: ExportContext) => void }) => ReturnType
```
...
| Property | Type    | Description                                       |
| -------- | ------- | ------------------------------------------------- |
| `blob`   | `Blob`  | The exported PDF file as a Blob (null on failure)  |
...
When you call `exportPdf`, the extension serializes the editor's document to JSON and sends it to the Tiptap convert service along with any configured options (style overrides, page layout, headers, footers). The service converts the document to PDF and returns it as a binary blob. The `onExport` callback receives this blob, which you can then download or process as needed.
