...
| `placeholders`       | `Object \| false` | Rename map (`{ page?, total? }`) for page-number tokens in headers/footers of any slot shape (plain text, stringified Tiptap JSON, or a Tiptap JSON node object), matching `Pages.configure({ placeholders })`. Pass `false` to disable substitution. See [Custom token names](/conversion/export/docx/rest-api#custom-token-names-placeholders). | `undefined` |
...
<Callout title="Accepted slot values">
    ...
    In all three shapes, `{page}`, `{total}`, and the `{numpages}` synonym render as live page-number fields; rename or disable them with the top-level `placeholders` property.
</Callout>
