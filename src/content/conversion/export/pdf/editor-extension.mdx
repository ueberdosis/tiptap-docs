...
<Callout title="On-premises only" variant="warning">
  Custom fonts are only available in on-premises deployments. If you're interested in using custom
  fonts with Tiptap's cloud service, please [contact us about our Enterprise
  plan](https://tiptap.dev/pricing).
</Callout>
...
- **Authentication required.** The export sends the document to the Convert service, which requires a signed token (`token`) carrying `aud: "Convert"`. Sign tokens on your server and never expose your signing key in the browser. See [Authentication](/authentication).
...
- **Custom fonts are an Enterprise / on-prem feature.** In the cloud plan, requests carrying `customFonts` return `403`. To enable them, contact us about an [Enterprise plan](https://tiptap.dev/pricing) or run an on-prem deployment.
