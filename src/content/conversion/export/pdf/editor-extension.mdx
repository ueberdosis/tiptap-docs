...
- **Authentication required.** The export sends the document to the Convert service, which requires a JWT (`token`) and your App ID. Generate JWTs server-side; never embed your secret key in the browser. See the [Conversion install guide](/conversion/getting-started/install).
