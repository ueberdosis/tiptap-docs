...
<Callout title="Requires Export DOCX extension" variant="warning">
  Style overrides for PDF export require the `@tiptap-pro/extension-export-docx` extension to be installed and configured in your editor. The PDF export uses DOCX generation under the hood before converting to PDF. Without it, the `styleOverrides` and element override options will have no effect. With it installed, the PDF also takes its styles from your editor's CSS in the browser, see [Document styles](/conversion/export/docx/document-styles). To turn that off, set `documentStyles: false` on `ExportDocx`.
</Callout>
