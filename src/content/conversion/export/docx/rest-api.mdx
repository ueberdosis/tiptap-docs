...
<Callout title="`{page}` / `{total}` tokens become live Word fields" variant="info">
  The REST API translates `{page}`, `{total}`, and the `{numpages}` synonym inside headers/footers
  into live Word `PAGE` / `NUMPAGES` fields in the exported `.docx`. This applies to every slot
  shape: plain-text strings and stringified Tiptap JSON alike. Custom token names (e.g. `{p}` /
  `{pages}`) require passing the optional `placeholders` rename map. See [Custom token
  names](#custom-token-names-placeholders) below. Pass `"placeholders": false` to ship the
