...
<Callout variant="warning" title="`{page}` / `{total}` tokens render as literal text on the REST API">
    The REST API does **not** translate `{page}` or `{total}` tokens into live Word `PAGE` / `NUMPAGES` fields — they render as the literal strings `{page}` / `{total}` in the exported `.docx`. Live page-number fields are produced only by the [editor extension](/conversion/export/docx/editor-extension) (when paired with the [Pages extension](/pages/getting-started/overview)) or by calling `convertHeader` / `convertFooter` directly with a `placeholders` option (see [Custom token names](/conversion/export/docx/headers-footers#custom-token-names)). For REST API workflows that need live page numbers in headers/footers today, run the export from the editor extension instead.
</Callout>
