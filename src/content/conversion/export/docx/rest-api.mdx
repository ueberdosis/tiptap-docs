...
<Callout title="`{page}` / `{total}` tokens become live Word fields" variant="info">
    The REST API translates `{page}`, `{total}`, and the `{numpages}` synonym inside plain-text headers/footers into live Word `PAGE` / `NUMPAGES` fields in the exported `.docx`. Custom token names (e.g. `{p}` / `{pages}`) require passing the optional `placeholders` rename map — see [Custom token names](#custom-token-names-placeholders) below. Pass `"placeholders": false` to ship the tokens as literal text instead.
</Callout>
