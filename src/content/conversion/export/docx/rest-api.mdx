...
<Callout title="`{page}` and `{total}` tokens render as live page-number fields">
    Plain-text headers and footers containing `{page}` or `{total}` are emitted as live `PAGE` / `NUMPAGES` fields in the exported `.docx`, so Word recalculates the actual current page and total page count on print or repagination. Renamed token names (configured via `Pages.configure({ placeholders })` in the editor) are an editor-side feature and are not currently configurable on the export REST API — pass the canonical `{page}` / `{total}` strings instead.
</Callout>
