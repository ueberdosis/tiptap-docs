...
| `pageFieldTokenNames`    | `Object` | Optional `{ page?, total? }` rename map matching `Pages.configure({ placeholders })` so renamed tokens (e.g. `{p}` / `{pages}`) in plain-text headers/footers become live `PAGE` / `NUMPAGES` fields. See [Custom token names](#custom-token-names-page-field-token-names). | `undefined` |
...
<Callout title="`{page}` / `{total}` tokens become live Word fields" variant="info">
    The REST API translates `{page}`, `{total}`, and the `{numpages}` synonym inside plain-text headers/footers into live Word `PAGE` / `NUMPAGES` fields in the exported `.docx`. Custom token names (e.g. `{p}` / `{pages}`) require passing the optional `pageFieldTokenNames` rename map — see [Custom token names](#custom-token-names-page-field-token-names) below.
</Callout>

#### Custom token names (`pageFieldTokenNames`)

When the editor renames the built-in tokens via [`Pages.configure({ placeholders })`](/pages/core-concepts/page-header-footer#renaming-or-disabling-placeholders), forward the same rename map to the REST API so the wire format stays aligned. The map is the same `{ page?, total? }` shape Pages accepts:

```json
{
  "doc": "{\"type\":\"doc\",\"content\":[]}",
  "footers": { "default": "{p} of {pages}" },
  "pageFieldTokenNames": { "page": "p", "total": "pages" }
}
```

When the map is omitted, the service falls back to recognizing `{page}`, `{total}`, and `{numpages}` (Word's spelling for the total-pages field, kept as a synonym so direct REST API callers can mirror what `/import/docx` emits). Renaming `total` removes the `{numpages}` synonym so stale tokens render literally — the rename is strict.
