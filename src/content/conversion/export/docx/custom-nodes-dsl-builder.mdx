...
This pairs naturally with CSS: let CSS handle headings, body fonts, lists, and other generic selectors; let the DSL handle anything tied to a custom node type. In the browser, the export reads your editor's CSS for those elements by default, see [Document styles](/conversion/export/docx/document-styles). [`cssStyles`](/conversion/export/docx/css-to-docx) adds rules you declare on top.
...
In the browser, the export already reads the rendered CSS for these generic elements, see [Document styles](/conversion/export/docx/document-styles). `extractFromDocument` in the example below adds the declared rules on top, and they win over the rendered values. Leave `cssStyles` out if the rendered CSS is what you want.
...
The builder is a thin authoring layer, so every runtime constraint described on the [JSON DSL reference page](/conversion/export/docx/custom-nodes-dsl#errors) applies identically:
