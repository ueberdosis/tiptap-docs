    ...
    ExportDocx.configure({
      ...
      customNodes: [
        {
          ...
          render: node => {
            ...
            // We return a Paragraph and apply our custom 'Hintbox' style to it.
            // The style name here must match the id defined in styleOverrides below.
      ...
      styleOverrides: {
        paragraphStyles: [
          ...
          // The id and name here become the Word paragraph style name in the exported DOCX.
          // If you plan to import this DOCX back into Tiptap, use this same id as the key
          // in prosemirrorNodes when configuring the ImportDocx extension.
          {
            id: 'Hintbox',
            name: 'Hintbox',
            basedOn: 'Normal',
...
<Callout title="Round-tripping custom nodes with ImportDocx" variant="info">
  If you plan to import the exported DOCX back into Tiptap using the `ImportDocx` extension, the
  `id` you set in `styleOverrides.paragraphStyles` (in this example `Hintbox`) is the key you will
  need to use in `prosemirrorNodes` on the import side. The key is case-sensitive and must match
  exactly.
</Callout>
...
ImportDocx.configure({
  prosemirrorNodes: {
    // key = style id from styleOverrides, value = Tiptap node name
    Hintbox: 'hintbox',
  },
})
```
