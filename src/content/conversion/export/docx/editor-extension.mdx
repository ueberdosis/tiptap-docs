...
| Custom node rendering | Supported via [`customNodes`](/conversion/export/docx/custom-nodes) (JavaScript render functions)                                                                                                                                                                                                    | Supported via the [custom-node DSL](/conversion/export/docx/custom-nodes-dsl) (`customNodeDsl`)\* |
| Element overrides     | [`paragraphOverrides`, `textRunOverrides`, `tableOverrides`, `tableRowOverrides`, `tableCellOverrides`, `imageOverrides`](/conversion/export/docx/styles#element-overrides), plus [`headerFooterOverrides`](/conversion/export/docx/styles#headerfooteroverrides) for header/footer-scoped overrides | Same fields except `tableRowOverrides`\*                                                          |
| Style overrides       | Supported                                                                                                                                                                                                                                                                                            | Supported                                                                                         |
| Headers and footers   | docx.js objects or auto-extracted from Pages                                                                                                                                                                                                                                                         | Plain text, stringified Tiptap JSON, or a Tiptap node object                                      |
...
\* The extension's `customNodes` option takes JavaScript render functions, which cannot be serialized into a request; the REST API covers the same ground declaratively with the [`customNodeDsl` body property](/conversion/export/docx/custom-nodes-dsl). Of the element overrides, only `tableRowOverrides` is not yet accepted by the REST API. Footnotes and endnotes remain extension-only on export for now.

Choose the REST API for server-side exports without running an editor: it covers style overrides, element overrides, and DSL-based custom nodes. Use the editor extension when you need function-based custom node rendering, `tableRowOverrides`, footnotes or endnotes, or full control over the output.
...
| **Table `width`**                         | A table with an explicit `width` exports at that fixed width; a table with no width exports full width (fluid) so it reflows to the page                                                                  |
| **Table borders**                         | The table's own borders (outer edges plus the inside horizontal and vertical grid) are exported; a borderless table stays borderless                                                                     |
...
| **Table cell margins**                    | `cellMarginTop` / `cellMarginRight` / `cellMarginBottom` / `cellMarginLeft` on the table node are exported as the table's cell margins. A table without them uses Word's default                          |
