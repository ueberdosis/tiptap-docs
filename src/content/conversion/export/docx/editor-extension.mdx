...
| numberingFormats      | Registry of multilevel ordered-list numbering format definitions. Each entry's `id` matches the `numberingFormat` attribute on the outermost `<ol>`; unknown / missing ids fall back to plain `1. 2. 3.` numbering. The export also includes each format that `OrderedListNumbering` from `@tiptap-pro/extension-convert-kit` knows. [See Ordered list numbering](/conversion/export/docx/ordered-list-numbering) | `undefined`                         |
