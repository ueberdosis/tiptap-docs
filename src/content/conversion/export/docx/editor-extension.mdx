...
| numberingFormats      | Registry of multilevel ordered-list numbering format definitions. Each entry's `id` matches the `numberingFormat` attribute on the outermost `<ol>`; unknown / missing ids fall back to plain `1. 2. 3.` numbering. Pair this with `OrderedListNumbering` from `@tiptap-pro/extension-convert-kit` and preview CSS from `generateNumberingFormatCss`. [See Ordered list numbering](/conversion/export/docx/ordered-list-numbering) | `undefined` |
