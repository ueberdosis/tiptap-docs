...
In the browser, the `exportDocx` command also reads the CSS your editor renders with, so the exported file looks like the editor. On a server, the export uses the package's default styles instead. See [Document styles](/conversion/export/docx/document-styles).
...
| Editor CSS            | Read by default in the browser. See [Document styles](/conversion/export/docx/document-styles)                                                                                                                                                                                                       | Not available                  |
...
| documentStyles        | Builds the Word styles from the CSS your editor renders with. It only runs in the browser, never on a server or with a headless editor. `cssStyles` and `styleOverrides` still win. [See Document styles](/conversion/export/docx/document-styles)                                                                                                                                                                | `true`                              |
...
| onWarning             | Called for each warning with its `code` and `message`, for example when the export skips an image or can't read the editor's CSS. Without it, warnings go to `console.warn`. [See Images that cannot be exported](#images-that-cannot-be-exported)                                                                                                                                                                | `undefined`                         |
...
| customNodes           | An array of custom node definitions. If your content includes custom nodes, pass their definitions here to ensure they're properly converted                                                                                                                                                                                                                                                                      | `[]`                                |
| styleOverrides        | An object with custom styles to apply to the exported document. [See Export styles](/conversion/export/docx/styles)                                                                                                                                                                                                                                                                                               | `{}`                                |
| documentStyles        | `false` skips reading the editor's CSS for this export. `true` reads it even when the extension turns it off. [See Document styles](/conversion/export/docx/document-styles)                                                                                                                                                                                                                                      | `true`                              |
| paragraphOverrides    | Base defaults for all paragraphs (also applied to header/footer content). Accepts a static object applied to every paragraph, or a function `(node) => overrides` that receives the current paragraph node. [See Element overrides](/conversion/export/docx/styles#element-overrides)                                                                                                                             | `{}`                                |
| textRunOverrides      | Base defaults for all text runs (also applied to header/footer content). Accepts a static object applied to every text run, or a function `(node) => overrides` that receives the current text node. [See Element overrides](/conversion/export/docx/styles#element-overrides)                                                                                                                                    | `{}`                                |
| tableOverrides        | Table-level property overrides (also applied to header/footer tables). Accepts a static object applied to every table, or a function `(node) => overrides` that receives the current table node. [See Element overrides](/conversion/export/docx/styles#element-overrides)                                                                                                                                        | `{}`                                |
| tableRowOverrides     | Base defaults for all table rows (e.g. `cantSplit`, `tableHeader`, `height`; also applied to header/footer tables). Accepts a static object applied to every row, or a function `(node) => overrides` that receives the current table row node. [See Element overrides](/conversion/export/docx/styles#element-overrides)                                                                                         | `{}`                                |
...
| Field         | Description                                                                                                                                                                                                                                                           |
| ------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `code`        | `image-missing-src`, `image-request-failed`, `image-fetch-failed`, or `image-unsupported-type` for a skipped image. `document-styles-failed` when the export could not read the editor's CSS, see [Document styles](/conversion/export/docx/document-styles#warnings) |
...
`onError` also catches failures that happen before the export writes the file, like an error while it collects comments. A failed CSS reading doesn't count as an error, though. The export reports it through `onWarning` and carries on with the default styles.
...
| **Table borders**                         | The table's own borders (outer edges plus the inside horizontal and vertical grid) are exported. A table without border attributes takes the borders your editor's CSS draws; without that reading it stays borderless |
...
| **Table cell margins**                    | `cellMarginTop` / `cellMarginRight` / `cellMarginBottom` / `cellMarginLeft` on the table node are exported as the table's cell margins. A table without them takes the cell padding from your editor's CSS, or Word's default without that reading |
...
- **Styles from your editor.** In the browser, the export reads your editor's CSS, so fonts, colors, spacing, list indents and table borders match what your users see. On a server, it uses the package's default styles. See [Document styles](/conversion/export/docx/document-styles).
