...
const editor = new Editor({
  extensions: [
    ...
    ExportDocx.configure({
      ...
      numberingFormats: [], // optional. Default: undefined
...
| numberingFormats      | Registry of multilevel ordered-list numbering format definitions. Each entry's `id` matches the `numberingFormat` attribute on the outermost `<ol>`; unknown / missing ids fall back to plain `1. 2. 3.` numbering. Editor-side wiring (attribute extension + preview CSS) is the consumer's responsibility. [See Ordered list numbering](/conversion/export/docx/ordered-list-numbering) | `undefined` |
...
<Callout title="Page-number token names are configured on Pages, not here">
    There is no `placeholders` option on `ExportDocx`. To rename the built-in `{page}` / `{total}` tokens or disable substitution, configure it on the [Pages extension](/pages/core-concepts/page-header-footer#renaming-or-disabling-placeholders) — the export honors the same token names, so the editor preview and the exported `.docx` stay consistent.
</Callout>
