...
<Callout title="Page-number token names are configured on Pages, not here">
    There is no `placeholders` option on `ExportDocx`. To rename the built-in `{page}` / `{total}` tokens or disable substitution, configure it on the [Pages extension](/pages/core-concepts/page-header-footer#renaming-or-disabling-placeholders) — the export reads the same configuration so the editor preview and the exported `.docx` stay consistent.
</Callout>
