...
Use the `pageColor` option to set a background color for every page in the exported document.
...
Pass `"transparent"` to omit the page color. If you leave `pageColor` undefined, an export in the browser uses the background your editor renders on. That's usually white, or dark if your app uses a dark color scheme. On a server, or with `documentStyles: false`, leaving it undefined means no page color. See [Document styles](/conversion/export/docx/document-styles#page-color).
