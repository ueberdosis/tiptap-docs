...
<Callout title="Auto-derived from the Pages extension" variant="hint">
  ...
  `ExportDocx`, the `header` and `footer` offsets are automatically derived from the Pages storage
...
Use the `pageColor` option to set a background color for every page in the exported document. The value is a CSS-like color string; hex values may include or omit the leading `#`.

### Accepted formats

| Format          | Example                                                  |
| --------------- | -------------------------------------------------------- |
| Hex (6-digit)   | `"#ff0000"`, `"FF0000"`                                  |
| Hex (3-digit)   | `"#f00"`                                                 |
| Hex with alpha  | `"#ff0000aa"` (alpha is ignored)                         |
| `rgb()`         | `"rgb(255, 0, 0)"`                                       |
| `rgba()`        | `"rgba(255, 0, 0, 0.5)"` (alpha is ignored)              |
| Named color     | `"red"`, `"blue"`, `"green"`, `"yellow"`, `"white"`, …   |

Supported named colors: `black`, `blue`, `brown`, `cyan`, `gray`, `green`, `indigo`, `lime`, `magenta`, `maroon`, `navy`, `olive`, `orange`, `pink`, `purple`, `red`, `teal`, `violet`, `white`, `yellow`.

Pass `"transparent"`, leave `pageColor` undefined, or use any unrecognized value to omit the page color entirely (the document opens with Word's default page color).
...
ExportDocx.configure({
  ...
  pageColor: '#fff8dc', // cornsilk-style cream page
})
```

<Callout title="Word does not print page backgrounds by default" variant="hint">
  In Word, the page background is shown on screen but is **not** included when printing or exporting
  to PDF. To make it visible in printed output or PDF exports, the reader must enable **File →
  Options → Display → Print background colors and images** in Word.
</Callout>
...
editor.commands.exportDocx({
  ...
  pageColor: 'gray', // overrides the configured page color for this export
})
```
