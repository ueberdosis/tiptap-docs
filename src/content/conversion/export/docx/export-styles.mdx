...
Accepts any `IParagraphOptions` property except `children`. These values are spread **first** as base defaults — per-node computed values (alignment, spacing, heading level, list numbering) override them.
