...
```ts
editor.commands.registerNumberingFormats(formatsFromTheImport)
```

Merging by id means repeating the call is safe: re-registering an id replaces that entry rather than adding a second one, so importing the same file twice leaves one definition behind. An id you configured in `formats` is replaced the same way, which lets an imported document override a house format of the same name.
...
```ts
ExportDocx.configure({ numberingFormats: editor.storage.orderedListNumbering.numberingFormats })
```

The `ImportDocx` extension calls `registerNumberingFormats` for you when `orderedListNumbering` is on, so an imported document renders its own markers without any wiring. Call it yourself when you load content some other way.
...
| `alignment`    | `'left'`                 | Where the marker is justified against `numberIndent`. See [Marker alignment](#marker-alignment).                                                                                                                                                                           |
...
| `textIndent`   | Word's per-level default | Distance from the page margin to the body text. Should be greater than `numberIndent`. A single item can sit somewhere else; see [Indents on a single item](#indents-on-a-single-item).                                                                                   |
...
When `numberIndent` / `textIndent` are omitted, the CSS helper emits Word's standard multilevel-list defaults, and so does the exporter unless something else places the list. [`listLevels`](#list-positions-with-listlevels) sets the indents for every depth. In the browser, the exporter also writes the indent each item has on screen, see [Document styles](/conversion/export/docx/document-styles#lists).
...
## List positions with `listLevels`

`listLevels` on `ExportDocx` sets where the marker and the text sit at each nesting depth of bullet, ordered and task lists. It works without a numbering format:
...
ExportDocx.configure({
  ...
  listLevels: {
    bullet: [
      { textIndent: 720, numberIndent: 360 },
      { textIndent: 1440, numberIndent: 1080 },
    ],
    ordered: [{ textIndent: '0.5in', numberIndent: '0.25in', format: 'lowerRoman' }],
    task: [{ textIndent: 900, numberIndent: 540 }],
  },
})
```
...
| Field          | Description                                                                                                                                |
| -------------- | ------------------------------------------------------------------------------------------------------------------------------------------ |
| `textIndent`   | Distance from the page margin to the body text. Twips number or a docx-style measure string (`'0.5in'`, `'1.27cm'`, `'36pt'`).             |
| `numberIndent` | Distance from the page margin to the marker. A marker left of the text becomes a hanging indent, a marker right of it a first-line indent. |
| `rightIndent`  | Distance from the right page margin to the end of the body text.                                                                           |
| `text`         | The marker text. On an ordered list without a numbering format, it replaces the counter, so the list stops counting.                       |
| `markerFont`   | Run formatting for the marker alone, as in [`NumberingMarkerFont`](#numberingmarkerfont).                                                  |
| `format`       | The counter of an ordered list without a numbering format, such as `'lowerRoman'`.                                                         |

- An array shorter than the nesting depth cycles, the same way a format's `levels` do.
- A format's own indents win over `listLevels`, and `listLevels` wins over Word's defaults. A list that names a format keeps its own counter and marker text, and takes `markerFont` from `listLevels` only when the format sets none.
- A task list uses only the indents, because the checkbox is its marker.
- A negative or non-finite value, or a string without a unit such as `'720'`, fails the export with an error that names the entry, for example `listLevels.bullet[1].textIndent`.
- In the browser, the export reads these values from your editor's CSS, see [Document styles](/conversion/export/docx/document-styles#lists). The `listLevels` you pass merge over the ones it read, per list kind and depth, and an item at a depth where you set an indent exports at your indent instead of its position on screen.
...
levels: [{ baseStyle: LevelFormat.UPPER_ROMAN, textTemplate: '%1.', alignment: 'right' }]
```

A `suffix` of `'space'` or `'nothing'` drops the padded marker column, so under `'left'` and `'center'` the body text follows the marker directly instead of waiting for `textIndent`. Combined with `'right'` the marker still ends at `numberIndent`, which leaves the body text starting there as well.

<Callout variant="info" title="An imported format carries no alignment">
...
Imported documents rely on this. The import writes each list paragraph's own Word indent onto the paragraph node as `indent` and `firstLineIndent` (see [ConvertKit](/conversion/import/docx/convertkit)), so the items land where the source document drew them rather than on the level's ladder.

Conditions and limits:

- It applies only to a list that names a format the editor knows about, whether from `formats` or from [`registerNumberingFormats`](#adding-formats-after-the-editor-is-running). Without one nothing positions the item, so the paragraph keeps rendering the indent itself as `padding-left` and `text-indent`, exactly as a paragraph outside a list does. The same holds when `orderedListNumbering` is off.
- A task list is not part of a numbering ladder, so an item in a checklist keeps its indent on the paragraph.
- A **positive** `firstLineIndent` is an ordinary first-line indent rather than a marker gap. It stays on the paragraph and indents the first line of the item's text.
- `indentRight` is left alone. A list positions the left edge of its items, not the right.
- A list item holding more than one block takes its position from the first block. The later blocks line up with it and do not render an indent of their own.
- The attributes stay on the node, so `ExportDocx` writes the same indent back out and a DOCX round trip keeps it.
- The editor positions the item and clears the paragraph's own `padding-left` and `text-indent` with an inline style. A rule of your own on that paragraph needs `!important` to take the block back.
...
| Per-item marker overrides (a different glyph or counter value on one item) | Use a separate list to start at a different counter value. Where an item *sits* is a separate matter; see [Indents on a single item](#indents-on-a-single-item). |
| DOCX → editor import                       | Handled by `@tiptap-pro/extension-import-docx`, which feeds the formats it reconstructs to the editor. See [Adding formats after the editor is running](#adding-formats-after-the-editor-is-running). |
