...
| -------------------------------------------------------- | ----------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `ExportDocx({ numberingFormats })`                       | `@tiptap-pro/extension-export-docx` | Pass the registry to the exporter so the `.docx` carries the matching definitions.                                                                                         |
| `generateNumberingFormatCss(formats)`                    | `@tiptap-pro/extension-export-docx` | Pure function that returns CSS text for the editor preview — same registry, same visual result.                                                                            |
| `NumberingFormatDefinition`, `NumberingLevelDefinition`  | `@tiptap-pro/extension-export-docx` | The data shape your registry follows.                                                                                                                                      |
...
| `OrderedListNumbering`                                   | `@tiptap-pro/extension-convert-kit` | Tiptap extension that adds the `numberingFormat` attribute to `orderedList` and exposes the `setOrderedListNumberingFormat(id)` command. Enforces outermost-only on paste. |
...
// 3. Register the schema attribute + register ExportDocx with the same registry.
...
`OrderedListNumbering` is registered by default when you use [`@tiptap-pro/extension-convert-kit`](/conversion/import/docx/convertkit). If you are not using the full `ConvertKit` bundle, you can register the extension directly:
...
const editor = new Editor({
  extensions: [
    StarterKit,
    OrderedListNumbering.configure({ formats: MY_FORMATS }),
    ExportDocx.configure({ numberingFormats: MY_FORMATS, onCompleteExport: ... }),
  ],
})
```

When you use `ConvertKit`, pass `orderedListNumbering: false` to disable it.
...
| `markerFont`   | —                        | Marker-only run formatting. Survives DOCX export and is reflected by `generateNumberingFormatCss` in the editor preview.                                                                |
