...
| `ExportDocx.configure({ numberingFormats })`                                   | `@tiptap-pro/extension-export-docx` | Extra formats for the `.docx`. The exporter already includes every format `OrderedListNumbering` knows.                                                                                                                                                                                                                                   |
...
//    and injects it for you, and ExportDocx exports the same formats.
...
The full registry, your configured formats plus everything registered since, is on the extension's storage as `editor.storage.orderedListNumbering.numberingFormats`. `ExportDocx` reads it on every export, so the round trip needs no wiring. On the same id, a registry format wins over one set in `ExportDocx.configure({ numberingFormats })`. A `numberingFormats` list passed to the `exportDocx` command wins over both, but it doesn't remove the registry's formats.
