---
title: Compile CSS rules into DOCX styles
tags:
  - type: start
  - type: beta
  - type: version
...
<Callout title="The export already reads your editor's CSS" variant="hint">
  Since v0.33.0, the `exportDocx` command reads the CSS your editor renders with and builds the DOCX
  styles from it, see [Document styles](/conversion/export/docx/document-styles). This page covers
  `cssStyles`, which compiles CSS rules that you pass or extract from stylesheets. Rules from
  `cssStyles` win over the rendered CSS, and `styleOverrides` wins over both.
</Callout>

## What this does

`cssStyles` lets you keep a **stylesheet** as the source of truth. You pass CSS rules, or let us extract them from the browser, and on export we compile them into DOCX **style definitions**, the named entries Word itself treats as the source of truth. The exported DOCX has a proper style catalog that Word users can edit once to change the look of every paragraph that uses the style.

In the browser, the export already builds that catalog from the CSS your editor renders with. `cssStyles` is for cases where the export can't read a rendered editor, like the standalone `exportDocx()` function on a server, or when you want a value pinned no matter what the page renders.
...
These limits only apply to `cssStyles`. The CSS reading described in [Document styles](/conversion/export/docx/document-styles) uses computed values, so CSS variables, `calc()`, inherited values and stylesheets from other origins work there.
...
- **Inline-mark properties surviving a round trip.** Export accepts all 12 properties on every selector, including `strong`, `em`, `u`, `s`, `a`, `code`. The matching [CSS injection](/conversion/import/docx/css-injection) on import filters those inline-mark selectors down to their identity properties only (e.g. `strong` keeps `fontWeight`, `em` keeps `fontStyle`, `code` keeps `fontFamily`/`color`/`fontSize`/`backgroundColor`). Anything you set beyond those is dropped on re-import to avoid overriding parent block styles via specificity. To round-trip, e.g., a colour on `strong`, set it on the parent selector (`p`, `h1`) instead.
...
- [Document styles](/conversion/export/docx/document-styles): the export reads the CSS your editor renders with, on by default in the browser.
- [CSS injection (import)](/conversion/import/docx/css-injection): the reverse direction.
- [Export styles](/conversion/export/docx/styles): the original DOCX export styles guide (manual style definitions, not CSS-based).
