...
<Callout title="The export already follows your editor's CSS" variant="hint">
  Since v0.33.0, the DOCX styles from the `exportDocx` command follow the CSS your editor renders
  with, see [Document styles](/conversion/export/docx/document-styles). This page covers
...
In the browser, that catalog already follows the CSS your editor renders with. `cssStyles` is for cases where the export can't use a rendered editor, like the standalone `exportDocx()` function on a server, or when you want a value pinned no matter what the page renders.
...
These limits only apply to `cssStyles`. [Document styles](/conversion/export/docx/document-styles) doesn't have them: CSS variables, `calc()`, inherited values and stylesheets from other origins all work there.
...
- [Document styles](/conversion/export/docx/document-styles): the export follows the CSS your editor renders with, on by default in the browser.
