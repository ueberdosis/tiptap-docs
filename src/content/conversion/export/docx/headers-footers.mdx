...
<Callout title="Tip: styling headers and footers" type="tip">
    `convertHeader` and `convertFooter` also accept `paragraphOverrides`, `tableOverrides`, `tableCellOverrides`, `textRunOverrides`, and `imageOverrides` as optional arguments — useful when you build headers and footers via factories and want them styled consistently with the body. If you instead let the Pages extension auto-extract headers and footers from the editor, use `headerFooterOverrides` on [`ExportDocx.configure()`](/conversion/export/docx/editor-export#configuring-the-extension) to style them without touching factories.
</Callout>
