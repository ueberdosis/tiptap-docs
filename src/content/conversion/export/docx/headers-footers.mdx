...
1. **`{page}` / `{total}` tokens in plain-text headers and footers** — works automatically with content extracted from the [Pages extension](/pages/getting-started/overview) and with plain-text strings passed to `headers` / `footers`. The export converts each token to a live `PAGE` / `NUMPAGES` field, so Word renders the actual current page and total page count.
...
If your editor renames the built-in tokens via `Pages.configure({ placeholders: { total: 'pages' } })` (see [Pages → renaming placeholders](/pages/core-concepts/page-header-footer#renaming-or-disabling-placeholders)), the export auto-picks up the same map — `{pages}` becomes a live `NUMPAGES` field, and the original `{total}` stops being recognized. There is no `placeholders` option on `ExportDocx` itself: configuring on Pages is the single source of truth so the editor preview and the exported `.docx` never drift apart.
