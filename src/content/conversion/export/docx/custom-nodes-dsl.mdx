...
| Render a custom node when calling any export REST endpoint (docx, pdf, odt, epub, doc).                                         | **`customNodeDsl`** (this page).                                          |
...
The most common shape instantiates a docx.js element:
...
`Table` children populate the docx.js `rows` argument, so you don't need to write `rows` yourself. A table requires at least one row (an empty table rejects at runtime).
...
| `node.attrs.<key>` | The named attribute (only one level deep, so `node.attrs.style.color` is **not** allowed in v1). |
...
There is no `concat` op; use `$template` for strings. There is no implicit numeric coercion either, so `{ "$op": "add", "args": ["3", 1] }` rejects with `DOCX_DSL_RUNTIME_TYPE_MISMATCH`.
...
2. the editor's rendered CSS              (documentStyles, in the browser)
3. cssStyles-derived styles               (the experimental cssStyles option)
4. styleOverrides                         (named DOCX styles)
5. global *Overrides options              (paragraphOverrides, textRunOverrides, …)
6. built-in Tiptap node/mark mapping      (the standard pipeline)
7. DSL child-pipeline mark overrides      (MarkPolicy.overrides[<mark>])
8. DSL applyMarks props on inline elements
9. explicit DSL element props
```

Layer 2 is on by default in the browser, see [Document styles](/conversion/export/docx/document-styles). It also adds element overrides of its own, which sit under yours in layer 5.

`inheritOverrides: false` on an `ElementNode` skips layer 5 for that element only, useful when you want a single hint-box paragraph to opt out of `paragraphOverrides` without affecting any other paragraph.
...
The DSL is accepted on every export REST endpoint (`/export/docx`, `/export/pdf`, `/export/odt`, `/export/epub`, `/export/doc`) as a sibling of the existing `styleOverrides`, `pageSize`, `pageMargins`, `headers`, and `footers` fields. See the [REST API reference](/conversion/export/docx/rest-api) for headers, response shape, and the rest of the body schema.
...
- [CSS to DOCX](/conversion/export/docx/css-to-docx): compose with the DSL so the `cssStyles` layer feeds heading and paragraph styles, while DSL rules cover your custom-node types.
