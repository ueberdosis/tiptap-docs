---
title: Export custom nodes to .odt
tags:
  - type: start
  - type: beta
  - type: version
...
The `@tiptap-pro/extension-export-odt` extension supports custom node conversion, allowing you to define how custom nodes in your Tiptap schema should be rendered in ODT exports.

<Callout title="Requires Export DOCX extension" variant="warning">
  Custom node conversion for ODT export requires the `@tiptap-pro/extension-export-docx` extension to be installed and configured in your editor. The ODT export uses DOCX generation under the hood before converting to ODT. Without it, the `customNodes` option will have no effect.
</Callout>

## Export custom nodes to .odt

Pass your custom node definitions via the `customNodes` option when configuring `ExportOdt`:
