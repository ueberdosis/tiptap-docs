---
title: Custom page layouts in ODT
tags:
  - type: start
  - type: beta
  - type: version
...
Use the `pageSize` option to define custom page dimensions for your exported ODT files. The page size configuration accepts width and height values with various measurement units.
...
<Callout title="Auto-derived from the Pages extension" variant="hint">
  ...
  `ExportOdt`, the `header` and `footer` offsets are automatically derived from the Pages storage
...
Use the `pageColor` option to set a background color for every page of the exported ODT. The value is a CSS-like color string; hex values may include or omit the leading `#`.
...
Pass `"transparent"`, leave `pageColor` undefined, or use any unrecognized value to omit the page color entirely.
...
Here's a complete example showing how to configure custom page layouts with the ODT Export extension:
