...
Use the `pageColor` option to set a background color for every page of the exported ODT.
...
Pass `"transparent"` to omit the page color. If you leave `pageColor` undefined and `ExportDocx` is installed, an export in the browser uses the background your editor renders on. That's usually white, or dark if your app uses a dark color scheme. Otherwise, leaving it undefined means no page color. See [Document styles](/conversion/export/docx/document-styles#page-color).
