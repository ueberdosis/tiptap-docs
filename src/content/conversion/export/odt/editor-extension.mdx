---
title: Export ODT from your editor
tags:
  - type: start
  - type: beta
  - type: version
...
You can also use the [REST API](/conversion/export/odt/rest-api) instead if you'd prefer to handle the conversion on your end.
...
The `ExportOdtCommandOptions` interface extends `ExportOdtOptions`, so every configuration option can be overridden per-command invocation:
...
- **Pixel-perfect Word-to-ODT layout match.** Use ODT when you want a portable open-format file; use [DOCX](/conversion/export/docx/editor-extension) when fidelity to Word is the priority.
