...
After the conversion concludes, create a Blob from the returned data and prompt the user to download the file.
...
The `exportDocx` method takes an `ExportDocxOptions` (`object`) as an argument with the following properties:
...
| options | An object to configure some parts of the export:<br/><br/>**exportType**: The type of data returned by the method:<br/>- **buffer**: Returns a Node.js `Buffer` (server-side only)<br/>- **stream**: Returns a Node.js `Stream` (server-side only)<br/>- **string**: Returns a `String`<br/>- **blob** (`default`): Returns a `Blob` |
...
In order to do so, you'd need to import the `exportDocx` function from the `@tiptap-pro/extension-export-docx` package, pass it your Tiptap JSON content, and return the resulting buffer to the client.
 ...
 * @example exportDocx({ document: editor.getJSON(), exportType: 'buffer', customNodes: [], styleOverrides: {} })
 */
...
app.post('/export-docx', async (req, res) => {
  try {
    ...
    const docxBuffer = await exportDocx(content)
