...
Use Tiptap’s `@tiptap-pro/beta-extension-export-docx` to export the editor's content as a `.docx` file. This extension integrates `DOCX` export functionality into your editor.
...
```bash
npm i @tiptap-pro/beta-extension-export-docx
```
...
```js
import { ExportDocx } from '@tiptap-pro/beta-extension-export-docx'
```
...
In order to do so, you'd need to import the `exportDocx` function from the `@tiptap-pro/beta-extension-export-docx` package, pass it your Tiptap JSON content, and return the resulting conversion to the client.
...
Here you have a simple example using `Express` and `@tiptap-pro/beta-extension-export-docx` on the server-side:
