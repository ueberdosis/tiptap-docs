...
One of the biggest advantages of the `@tiptap-pro/beta-extension-export-docx` and `@tiptap-pro/beta-extension-import-docx` extensions is the ability to define how custom nodes in your Tiptap schema should be rendered in DOCX.
...
import {
  ...
  pixelsToHalfPoints,
  pointsToTwips,
} from '@tiptap-pro/beta-extension-export-docx'
...
You can construct any supported `DOCX` elements in the `render` function using the `Docx` library classes (`Paragraph`, `TextRun`, `Table`, etc.) that are provided via the `Docx` import from the `@tiptap-pro/beta-extension-export-docx` package.
...
```ts
import { Import } from '@tiptap-pro/beta-extension-import-docx'
