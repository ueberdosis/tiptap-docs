...
In the example above ☝️ we are exporting a document with a custom `Heading 1` style. The style is based on the `Normal` style, has a red color, and uses the `Aptos Light` font. The spacing before the paragraph is set to `12pt`, and `6pt` after it. The line height is set to `1.15`.
