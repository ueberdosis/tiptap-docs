...
In the browser, these styles follow the CSS your editor renders with, so Normal, the headings, Quote, List Paragraph, Hyperlink and Code look the way they do in the editor. See [Document styles](/conversion/export/docx/document-styles). The table lists the definitions the export falls back to on a server, with `documentStyles: false`, or with a headless editor:
...
Override any default style or add new ones using `styleOverrides`. Your overrides are deep-merged with the defaults: matching `id` values merge recursively, new `id` values are appended. In the browser, they merge over the styles taken from your editor's CSS the same way.
    ...
    paragraphStyles: [
      {
        ...
        paragraph: {
          spacing: {
            ...
            lineRule: 'auto', // makes `line` a multiple; styles from your CSS use 'atLeast'
          },
        },
      },
    ],
