...
Style resolution on import works identically via both the [editor extension](/conversion/import/docx/editor-extension) and [REST API](/conversion/import/docx/rest-api). For export, both paths support [`styleOverrides`](/conversion/export/docx/styles) for customizing document-level style definitions. The extension additionally supports [element overrides](/conversion/export/docx/styles#element-overrides) for per-element control that the REST API does not accept.
...
In the browser, the editor extension builds these styles from the CSS your editor renders with, so Normal, the headings, Quote, List Paragraph, Hyperlink and Code look the way they do in the editor. See [Document styles](/conversion/export/docx/document-styles). The table lists the definitions the export falls back to on a server, with `documentStyles: false`, or with a headless editor:

| Style | Font | Size | Other |
| --- | --- | --- | --- |
| Normal | Arial | 11pt | 10pt after-spacing, 1.15 line height |
| Heading 1 | Arial | 16pt | Bold, color #2E74B5 |
| Heading 2 | Arial | 14pt | Bold, color #2E74B5 |
| Heading 3 | Arial | 13pt | Bold, color #2E74B5 |
| Heading 4 | Arial | 12pt | Bold, color #2E74B5 |
| Heading 5 | Arial | 11pt | Bold, color #2E74B5 |
| Title | Arial | 22pt | Bold, black, centered |
| Subtitle | Arial | 16pt | Italic, gray, centered |
| Quote | Arial | (inherits) | Italic, centered |
| Hyperlink | (inherits) | (inherits) | Color #0563C1, underline |

### Customizing export styles

Override any default style or add new ones using `styleOverrides`. Your overrides are deep-merged with the defaults: matching `id` values merge recursively, new `id` values are appended. In the browser, they merge over the styles read from your editor's CSS the same way.
    ...
    paragraphStyles: [
      {
        ...
        paragraph: {
          spacing: {
            ...
            lineRule: 'auto', // makes `line` a multiple; the CSS reading writes 'atLeast'
          },
        },
      },
    ],
...
<Callout title="Units in style definitions" variant="info">
  Font sizes are in half-points (24 = 12pt). Spacing values are in twips (20 twips = 1pt). Line spacing uses 240ths of a line (240 = single, 276 = 1.15) when `lineRule` is `'auto'`. With `'atLeast'` or `'exact'`, `line` is in twips. These units come from the `docx` library and match the OOXML specification.
</Callout>
