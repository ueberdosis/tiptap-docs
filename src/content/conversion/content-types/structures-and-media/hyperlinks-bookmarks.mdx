...
- **Extensions:** [ConvertKit](/conversion/import/docx/convertkit), which registers both the `Link` mark for hyperlinks and the `Bookmark` node for anchor targets.
...
ConvertKit registers the [`Link`](/editor/extensions/marks/link) extension, so no separate installation is needed.
...
An external hyperlink with plain link text survives the full import, edit, export cycle. The href and visible text are preserved. The exported link uses the DOCX Hyperlink style. In the browser, that style picks up the color and underline your CSS uses for links, see [Document styles](/conversion/export/docx/document-styles). Otherwise links come out blue and underlined.
