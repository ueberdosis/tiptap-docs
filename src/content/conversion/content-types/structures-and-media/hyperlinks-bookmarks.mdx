...
- `StarterKit` includes the `Link` extension. No additional installation required.
- External links (http, https, mailto) work out of the box
- Internal bookmark links (using `w:anchor`) are not imported — the text is preserved but the link is lost
...
<CodeDemo src="http://localhost:3000/preview/Extensions/ContentTypeHyperlinksBookmarksImport?hideSource=true" />
...
<CodeDemo src="http://localhost:3000/preview/Extensions/ContentTypeHyperlinksBookmarksEditor?hideSource=true" />

The [`Link` extension](/editor/extensions/marks/link) is included in StarterKit. It renders link marks as `<a>` tags with the `href` attribute.
...
<CodeDemo src="http://localhost:3000/preview/Extensions/ContentTypeHyperlinksBookmarksExport?hideSource=true" />
