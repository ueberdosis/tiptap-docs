...
A table with an explicit width exports at that fixed width; a table with no width set exports full width (fluid) so it reflows to the page. Column widths come from each cell's `colwidth`, or are split evenly when none is set.
...
<Callout title="Cell and table styling round-trips on export" variant="info">
  Table borders (outer edges and the inside grid), cell backgrounds, per-cell borders, vertical alignment, and row height are written to the DOCX on export. A borderless table stays borderless. You can still set document-wide defaults with [`tableOverrides` and `tableCellOverrides`](/conversion/export/docx/styles#element-overrides) on the editor extension; these are not available via the REST API.
</Callout>
...
These overrides apply document-wide as defaults. Each cell's own styling (background, per-side borders, vertical alignment) and the table's own borders are exported from the node attributes on top of those defaults.
