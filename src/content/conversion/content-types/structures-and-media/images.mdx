...
| Image cropping | Supported | Requires custom handling | Not supported | `cropTop`, `cropBottom`, `cropLeft`, `cropRight` extracted as attributes (percentage values) |
...
If the source image has cropping applied in Word, the crop values are extracted as `cropTop`, `cropBottom`, `cropLeft`, and `cropRight` attributes (percentage values from 0 to 100). The default Image extension does not render cropping, so displaying cropped images requires custom handling (e.g., using CSS `clip-path` or `object-position`). Crop values are not preserved on export.
