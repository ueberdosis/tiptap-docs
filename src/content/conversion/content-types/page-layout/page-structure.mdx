...
Page numbers in Word are field codes (`PAGE`, `NUMPAGES`) inside headers and footers. The import pipeline translates these into canonical text tokens — `{page}` for `PAGE` and `{numpages}` for `NUMPAGES` — when the Pages extension is installed (or you pass `pageFieldTokens: true` explicitly). On export, plain-text `{page}` / `{total}` tokens in header/footer content are emitted as live `PAGE` / `NUMPAGES` fields so Word recalculates them on print or repagination.
...
| Page numbers | Yes | Word `PAGE` / `NUMPAGES` ↔ `{page}` / `{numpages}` (Pages canonical name for total is `{total}`; configure `placeholders: { total: 'numpages' }` to bridge) |
