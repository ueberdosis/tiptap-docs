...
| Page number placeholders | Not converted from DOCX (PAGE/NUMPAGES fields import as their cached display value, e.g. the literal text `1`) | Supported (`{page}`, `{total}` substituted at render time by the Pages extension) | Pass through as literal text. The export does not emit DOCX PAGE/NUMPAGES fields, so a header `Page {page} of {total}` appears in the exported DOCX as the string `Page {page} of {total}`. |
...
Page numbers are rendered through placeholders: `{page}` for current page, `{total}` for total page count. These placeholders are an editor-side feature: the Pages extension substitutes them when rendering each page. They are **not** translated into DOCX page-number fields on export. See the warning in the [Export](#export) section below.
...
<Callout variant="warning" title="`{page}` and `{total}` are not converted to DOCX page-number fields">
  The Pages extension's `{page}` and `{total}` placeholders are substituted when rendering pages in the editor, but the export does not detect them. Whatever string the editor produces (including `{page}` / `{total}` if the export reads from raw header content rather than rendered output) lands in the DOCX as literal text. Word will display the curly-brace tokens themselves, not a dynamic page count. To get a live page-number field in the exported DOCX today, supply a `Docx.Header` / `Docx.Footer` instance manually with the appropriate `docx.js` field-code primitives instead of relying on the Pages-extension auto-extraction.
</Callout>
