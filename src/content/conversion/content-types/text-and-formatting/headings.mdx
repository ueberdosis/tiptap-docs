...
The standard `Heading` extension's schema only defines `level`. With ConvertKit installed, the custom Heading additionally renders `spacingBefore`, `spacingAfter`, `lineHeight`, `fontSize`, `indent`, `indentRight`, `firstLineIndent`, and `contextualSpacing`. Without ConvertKit, those attributes are stripped on `setEditorContent()`.
