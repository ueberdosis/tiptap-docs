...
The standard `Heading` extension's schema only defines `level`. With ConvertKit installed, the custom Heading additionally renders `spacingBefore`, `spacingAfter`, `lineHeight`, `fontSize`, `indent`, `indentRight`, `firstLineIndent`, and `contextualSpacing`, and keeps `keepNext` and `keepLines` in the JSON. Without ConvertKit, those attributes are stripped on `setEditorContent()`.
...
In the browser, the editor extension exports each heading level with the font, size, color and spacing your editor's CSS gives `h1` to `h6`. See [Document styles](/conversion/export/docx/document-styles). On a server, with `documentStyles: false`, or with a headless editor, the export falls back to default heading styles: Arial, blue #2E74B5, and a size per level. To follow a corporate template or brand instead, customize the headings via `styleOverrides`:
...
What changes: the exported heading takes its look from your editor's CSS, or from the export's defaults when your CSS isn't available, plus your overrides. The original DOCX's theme styles don't come along, so the original font, size, spacing and color are lost.
