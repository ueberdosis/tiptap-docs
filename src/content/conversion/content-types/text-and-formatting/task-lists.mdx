...
Items use Word's List Paragraph style with a hanging indent. In the browser, the editor extension places each item at the indent your editor draws it, so a checkbox list can sit at a different indent than a bullet list. See [Document styles](/conversion/export/docx/document-styles#lists). Through the REST API, on a server, or with `documentStyles: false`, a checkbox list sits at the same indent as a bullet or numbered list in the same document, with the same spacing between rows. To set the indents yourself, use [`listLevels.task`](/conversion/export/docx/ordered-list-numbering#list-positions-with-listlevels).

Nested task lists export with the deeper indent your editor draws, or, without your CSS, the one Word uses for a nested list. When a task item holds several blocks, the checkbox goes on the first one and the rest are indented to match without a second checkbox.
