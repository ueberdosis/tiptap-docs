...
<Callout title="Numbering format and start values are not exported" variant="warning">
  The export converter does not read the `type` or `start` attributes from ordered list nodes. All
  ordered lists export as default decimal numbering (1, 2, 3) regardless of the format set in the
  editor. A list with `type: "a"` and `start: 5` in the editor will export as a standard 1, 2, 3
  list in Word.
</Callout>
