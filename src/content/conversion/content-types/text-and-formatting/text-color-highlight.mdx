...
The [`Color`](/editor/extensions/marks/color) extension adds color support to [`TextStyle`](/editor/extensions/marks/text-style), and the [`Highlight`](/editor/extensions/marks/highlight) extension handles background colors. ConvertKit bundles both: `TextStyleKit` brings `TextStyle` + `Color` + `BackgroundColor`, and `Highlight` is registered with `multicolor: true` preconfigured.
