...
Your endpoint should answer within 10 seconds. A slower answer counts as a failed upload for that image. Up to three uploads run at the same time, so it needs to handle concurrent requests.

Every image in the document is posted, whatever its type. `Content-Type` is set from the file extension for `png`, `jpeg`, `gif`, `bmp`, `tiff`, `webp`, `svg`, `emf` and `wmf`. Anything else is sent as `application/octet-stream`, so decide on your side which types you keep. There is no limit on the size of a single image or on the number of images in a document.
...
That image is skipped and the rest of the document imports normally. The same happens when the upload itself fails, times out, or returns something that is not JSON. A failed upload is not retried, and it never fails the import.
