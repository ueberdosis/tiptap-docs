...
- **`File-Name` comes from inside the DOCX**, where Word numbers pictures `image1`, `image2` and so on, restarting at 1 in every document. Two imports send the same name for unrelated images, so generate your own storage key rather than using this one. Otherwise the second import overwrites the first document's images, and that document then shows the wrong pictures.
...
```ts
 ...
 app.post('/upload', async (c) => {
   ...
   // Word names pictures image1, image2 and so on in every document, so
   // `File-Name` collides across imports. Store under a key of your own.
