...
- 32 bundled extensions wired up to handle everything the Convert API produces.
...
ConvertKit ships **32** extensions inside the single `@tiptap-pro/extension-convert-kit` package. The list below is reference material so you know what shows up in the editor's schema; you do not install or register these separately.
...
| Lists      | `BulletList`, `OrderedList`, `ListItem`, `ListKeymap`, `TaskList`, `TaskItem`, `OrderedListNumbering`†                                                              |
| Media      | `Image`\*                                                                                                                                                           |
...
| --------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `Paragraph`                 | `spacingBefore`, `spacingAfter`, `lineHeight`, `fontSize`, `indent`, `indentRight`, `firstLineIndent`, `contextualSpacing`, `keepNext`, `keepLines`                                                                                                                    | DOCX `w:pPr` + `w:rPr` on paragraph mark                                                                                                                    |
...
| `Table`                     | `width`, `indent`, per-side table border (`width` / `style` / `color` on top/bottom/left/right plus the inside horizontal and vertical grid), `cellMarginTop`, `cellMarginRight`, `cellMarginBottom`, `cellMarginLeft` (+ `cellMinWidth: 1` default, vs Tiptap's `25`) | DOCX `w:tblW`, `w:tblInd`, `w:tblGrid`, `w:tblBorders`, `w:tblCellMar`                                                                                      |
...
- **List indents position the item, not the paragraph.** With `orderedListNumbering` on, the `indent` on an imported list paragraph moves the whole list item, marker and text together, rather than padding the paragraph inside it. Imported items then sit where Word drew them instead of drifting away from their own marker at each nesting level. See [Indents on a single item](/conversion/export/docx/ordered-list-numbering#indents-on-a-single-item) for the conditions and for what stays on the paragraph.
