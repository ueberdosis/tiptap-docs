...
When the imported document contains numbered lists, the convert service reconstructs each list's numbering format — lettered, Roman, multilevel outlines, locale styles, and custom start values — and returns the definitions alongside the content. See [Lists](/conversion/content-types/text-and-formatting/lists) for what is preserved.

If the [`OrderedListNumbering`](/conversion/export/docx/ordered-list-numbering) extension is installed (opt in via `ConvertKit.configure({ orderedListNumbering: true })`), `setEditorContent()` registers the imported definitions automatically and the lists render their true markers right away — no extra wiring needed.

The definitions are also available on the `onImport` context as `numberingFormats`, in the same shape `ExportDocx` accepts. Hand them to the exporter for a faithful round-trip; `editor.storage.orderedListNumbering.numberingFormats` always holds the current merged set:
...
editor
  ...
  .importDocx({
    ...
    onImport(context) {
      ...
      // Registers context.numberingFormats with OrderedListNumbering
...
// Later, export with the formats the import registered:
await exportDocx({
  document: editor.getJSON(),
  exportType: 'blob',
  numberingFormats: editor.storage.orderedListNumbering.numberingFormats,
})
```

Without the `OrderedListNumbering` extension, lists still import with baseline `type` (`a`/`A`/`i`/`I`) and `start` attributes where the format has an HTML equivalent, so the browser's native `<ol>` rendering approximates the document.

## Footnotes & Endnotes

<Callout
  variant="warning"
  title="Footnotes and endnotes are not available in the extension's onImport callback"
>
  ...
  API](/conversion/import/docx/rest-api) directly and extract the `footnotes` and `endnotes` fields
  from the response.
</Callout>
