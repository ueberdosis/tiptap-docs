...
Word headers and footers commonly contain `PAGE` and `NUMPAGES` field codes that render as live page numbers. By default, when the [Pages extension](/pages/getting-started/overview) is installed, `importDocx` translates those fields into canonical text tokens — `{page}` for `PAGE` and `{numpages}` for `NUMPAGES` — so they round-trip cleanly back to live Word fields on re-export.

The default Pages config recognizes `{page}` (it's the default name for the `page` token) but **not** `{numpages}` (Pages' default name for the total-pages field is `total`). To make Word's `NUMPAGES` field render live in the editor preview after an import, point Pages at the canonical name:

```ts
import { Pages } from '@tiptap-pro/extension-pages'

Pages.configure({
  placeholders: { total: 'numpages' },
})
```

If you prefer a different name (e.g. `{pages}`), do the rewrite yourself in the `onImport` callback before calling `setEditorContent` — the imported header/footer JSON is plain text, so a single `.replace(/\{numpages\}/g, '{pages}')` walk is enough.

#### Disabling token translation

Pass `pageFieldTokens: false` on the import command to opt out. Word's cached numeric value flows through as plain text instead — the historical behavior, useful when no Pages extension is installed downstream:

```ts
editor.chain().importDocx({ file, pageFieldTokens: false }).run()
```

When `pageFieldTokens` is omitted, the command opts in by default whenever the Pages extension is installed — unless you've disabled token substitution with `Pages.configure({ placeholders: false })`, in which case it stays off. Pass `true` explicitly to force translation on regardless.
