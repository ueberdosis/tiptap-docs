...
When `pageFieldTokens` is omitted, the command auto-detects: enabled if the Pages extension is registered AND `placeholders` is not set to `false`; disabled otherwise. Pass `true` explicitly to force translation on regardless.
