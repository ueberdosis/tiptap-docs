...
4. **Auto-inject or hand-off.** If `autoInject: true`, the extension appends a scoped `<style>` element to `document.head`. If `autoInject: false`, you receive the raw `cssStyles` object on the `onImport` callback and decide what to do with it. In environments without a DOM (server-side rendering or headless editors) injection and its cleanup are safely skipped; the `cssStyles` object still reaches your callback.
...
- [CSS to DOCX (export)](/conversion/export/docx/css-to-docx): the reverse direction. Compiles your editor's CSS back into DOCX style definitions.
