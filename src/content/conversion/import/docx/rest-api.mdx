...
Every measurement is in twips, while Pages' `setPageFormat` expects pixels and an `id`. Use `twipsToPixels` from Convert Kit's OOXML registry rather than writing the conversion yourself, so your numbers match the ones the conversion service uses:
...
`prosemirrorMarks` accepts a second kind of entry. When the key is the name of a Word character style rather than one of the marks above, any run using that style receives the mark you name. For example, `"{\"Strong\":\"bold\"}"` gives every run styled `Strong` a `bold` mark.
