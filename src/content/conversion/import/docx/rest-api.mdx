...
<Callout variant="info" title="Use the shared unit helpers">
  The OOXML registry exports the conversions both sides of the pipeline use, including `twipsToPixels`, `pixelsToTwips`, `halfPointsToPixels`, `emuToPixels` and `twipsToPoints`, along with constants such as `TWIPS_PER_PIXEL`. Reaching for them keeps your numbers identical to the importer's and the exporter's. Rolling your own arithmetic is the usual reason a document looks slightly off after a round-trip.
</Callout>
...
<Callout variant="info" title="Code blocks and inline code cannot be renamed yet">
  The code block type and the inline code mark are always imported as `codeBlock` and `code`. Renaming them through `prosemirrorNodes` or `prosemirrorMarks` has no effect at the moment. If your schema uses different names, map them after the import.
</Callout>
