...
| `pageFieldTokenNames` | `JSON string` | Optional rename map for the emitted tokens when `pageFieldTokens=true`. JSON-encoded `{ "page"?: string, "total"?: string }` — e.g. `{"total":"pages"}` makes `NUMPAGES` come back as `{pages}` instead of the wire-default `{numpages}`. The editor extension forwards `Pages.options.placeholders` here automatically so the editor preview substitutes the imported tokens natively. |
...
Downstream consumers (such as the [Pages extension](/pages/getting-started/overview)) substitute `{page}` and `{numpages}` whatever token names they're configured to recognize. To make the wire format match a custom token registry, pass `pageFieldTokenNames` alongside `pageFieldTokens`:
...
curl -X POST "https://api.tiptap.dev/v2/convert/import/docx" \
    ...
    -F "file=@/path/to/your/file.docx" \
    -F "pageFieldTokens=true" \
    -F 'pageFieldTokenNames={"page":"page","total":"total"}'
```

The example above maps `NUMPAGES` to `{total}` (Pages' default name for the total-pages field), so the editor preview substitutes natively without any config. The [editor extension](/conversion/import/docx/editor-extension) forwards `Pages.options.placeholders` here automatically when the Pages extension is installed.
