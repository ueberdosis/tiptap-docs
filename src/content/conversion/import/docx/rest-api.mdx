...
Every measurement is in twips. Pages' `setPageFormat` expects pixels and an `id`, so divide by 15 to convert (1 twip is 96/1440 of a pixel at 96 DPI) and drop the header and footer margins, which `setPageFormat` does not take:
...
editor.commands.setPageFormat({
  id: 'docx-import',
  width: twipsToPx(pageLayout.width),
  height: twipsToPx(pageLayout.height),
  margins: {
    top: twipsToPx(pageLayout.margins.top),
    right: twipsToPx(pageLayout.margins.right),
    bottom: twipsToPx(pageLayout.margins.bottom),
    left: twipsToPx(pageLayout.margins.left),
  },
})
```

The [editor extension](/conversion/import/docx/editor-extension) does this for you when the Pages extension is registered, so this conversion is only needed on the REST path.
