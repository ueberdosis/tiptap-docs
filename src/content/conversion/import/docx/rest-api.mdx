...
| `taskLists`         | `"import" \| "ignore"` | Whether Word checkbox lists become `taskList` / `taskItem` nodes. Defaults to `"ignore"`, which imports them as bullet or numbered lists. Send `"import"` when your editor registers the task nodes. See [Task lists](/conversion/content-types/text-and-formatting/task-lists).                                                                                                                                                                                                                                                                                                                             |
...
Register `numberingFormats` with the editor so the lists keep their numbering. `ExportDocx` exports the registered formats too, so a round-trip preserves it. The [list numbering and bullets](/conversion/export/docx/ordered-list-numbering) page covers the full cycle.
