...
Every response carries a `logs` object next to `data`, holding `warn` and `error` arrays. You do not
have to ask for it. A warning means content did not make it into the document, so it is worth
surfacing to whoever uploaded the file.
...
<Callout title="`verbose` was removed" variant="info">
  Logs used to sit behind a `verbose` bitmask, and the default of `0` meant nothing was recorded at
  all. Warnings and errors now always come back. Requests that still send `verbose` are accepted and
  the value is ignored, so nothing breaks.
</Callout>
