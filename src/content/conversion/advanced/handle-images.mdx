...
When converting documents, some formats may include images that you may want to preserve in the converted document.
...
If you import a DOCX or ODT file that has images, the conversion service can include those images in the resulting Tiptap JSON **only if you provide an image upload callback URL**.
...
## Export image handling
The `src` attribute of each image node must be an accessible URL. If the converter cannot reach the image (e.g., a local or firewall-protected link), it **won’t** appear in the export.

- **DOCX/ODT:** The service fetches each image from its `src` URL and embeds it directly in the exported file, so the document is self-contained.
- **Markdown:** Images remain referenced by their external URLs (`![Alt Text](https://example.com/image.png)`).