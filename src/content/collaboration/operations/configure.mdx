...
- `webhook_awareness`: awareness webhooks (1 for enabled, 0 for disabled): user.connected, user.disconnected
- `webhook_retries`: enable webhook retries (1 for enabled, 0 for disabled)
...
- `webhook_log_errors_only`: turn off webhook success logs
- `allowed_origins`: If set, this will validate `Origin` headers from clients to match with one of the provided values (comma separated). Example value: `https://test.tiptap.dev,https://prod.tiptap.dev` ; if not set, origin header validation is turned off.
...
```bash
curl --location --request PUT 'https://YOUR_APP_ID.collab.tiptap.cloud/api/admin/settings/:key' \
--header 'Content-Type: text/plain' --header 'Authorization: YOUR_SECRET_FROM_SETTINGS_AREA' \
-d 'your value'
