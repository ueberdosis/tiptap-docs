...
Tiptap Collaboration lets teams edit the same document at the same time, even when network conditions are poor.

It runs on our open source [Hocuspocus](https://github.com/ueberdosis/hocuspocus) backend, syncs content with the Yjs CRDT, and scales from a single demo to millions of concurrent connections.

## Get started
Create a [Tiptap account](https://cloud.tiptap.dev) to integrate or test the Collaboration backend for free.

1. **Create an app** – log in to your [Tiptap account](https://cloud.tiptap.dev), start your app and copy your AppID plus the test token.
2. **Add the packages** – follow the [installation guide](/collaboration/getting-started/install).
