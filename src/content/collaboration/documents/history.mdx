...
When auto versioning is enabled, new versions are created at regular intervals (30 seconds by default, only if the document has changed). Note that this can create quite a lot of versions, so you might want to increase the interval.
