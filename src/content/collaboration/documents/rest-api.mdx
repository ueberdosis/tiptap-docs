...
With [Tiptap Semantic Search](/collaboration/documents/semantic-search) enabled, you can easily search across all your documents.

<Callout title="Keeping your API key secret" variant="hint">
  Please make sure that you handle the search requests in your own backend in order to keep your API
  key secret. This is also a good time to enforce a rate limit in your application.
</Callout>

### Query parameters

You can use the following query parameters to steer the search results to your likings.

| Query parameter | Type  | Default | Description                                                                       |
| --------------- | ----- | ------- | --------------------------------------------------------------------------------- |
| `threshold`     | float | `0.5`   | Describes the similarity factor of documents. The value can be betweem `0` an `1` |
| `limit`         | int   | `20`    | Limit the number of results. The value can be betweem `1` an `100`                |

### Body parameters

| Body parameter | Type   | Default | Description      |
| -------------- | ------ | ------- | ---------------- |
| `content`      | string | -       | Your search term |
