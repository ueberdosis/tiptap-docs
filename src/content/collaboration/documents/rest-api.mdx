...
Authenticate your API requests with a signed token carrying the `Documents:Api:All` permission, sent as `Authorization: Bearer <jwt>`. See [Authentication](/authentication) for how to sign a token.

<Callout title="Keep this token server-side" variant="warning">
...
The previous API secret still works and is documented under [Legacy authentication](/authentication/legacy).
...
Every version's `meta` object automatically includes a `__tiptap` key with server-generated metadata about who contributed changes and how the version was created. See [automatic version metadata](/collaboration/documents/snapshot#automatic-version-metadata) for details.
