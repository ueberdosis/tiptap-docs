...
| changesBy   | `string[]` | A list of user identifiers who made changes included in this version. The identifiers are the `user` values set on the `TiptapCollabProvider` of the contributing clients. |
| triggeredBy | `string`   | The user identifier of the person who triggered the version creation. For `"websocket"` versions this is the authenticated JWT `sub`; for `"api"` versions it is the `user` query parameter supplied by your backend. |
...
<Callout title="Securing editing attribution" variant="warning">
  By default, `changesBy` reflects the `user` value each client sets on its `TiptapCollabProvider`. This is a client-supplied value and is **not** verified against the authenticated JWT, so a client could record changes under another user's identity.

  To make attribution trustworthy, enable the [`user_mapping_authenticator`](/collaboration/operations/configure) setting. When enabled, the server rejects any attempt to register, overwrite, or delete a user identity that does not match the connection's JWT `sub`. With this setting on, **every client must set `TiptapCollabProvider.user` equal to its JWT `sub`**, otherwise its connection is rejected. This requires JWT authentication to be enabled.
</Callout>
