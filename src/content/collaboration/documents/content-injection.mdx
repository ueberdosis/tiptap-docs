...
<Requirements>
    ...
    <RequirementItem label="3. Authenticate">
        Sign a token carrying `Documents:Api:All` and send it as `Authorization: Bearer <jwt>`. See [Authentication](/authentication). Keep this token server-side.
    </RequirementItem>
</Requirements>
