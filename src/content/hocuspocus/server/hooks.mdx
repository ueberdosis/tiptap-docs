...
title: Hooks
meta:
  title: Hocuspocus | Hocuspocus Docs
  category: Hocuspocus
---

## Introduction

Hocuspocus offers hooks to extend its functionality and integrate it into existing applications. Hooks are configured as simple methods the same way as [other configuration options](/hocuspocus/server/configuration) are.

Hooks accept a hook payload as first argument. The payload is an object that contains data you can use and manipulate, allowing you to built complex things on top of this simple mechanic, like [extensions](/hocuspocus/guides/custom-extensions).
...
| `onAuthenticate`           | When authentication is required           | [Read more](/hocuspocus/server/hooks#on-authenticate)            |
| `onAwarenessUpdate`        | When awareness changed                    | [Read more](/hocuspocus/server/hooks#on-awareness-update)        |
| `onLoadDocument`           | During the creation of a new document     | [Read more](/hocuspocus/server/hooks#on-load-document)           |
| `afterLoadDocument`        | After a document is created               | [Read more](/hocuspocus/server/hooks#after-load-document)        |
| `onChange`                 | When a document has changed               | [Read more](/hocuspocus/server/hooks#on-change)                  |
| `onDisconnect`             | When a connection was closed              | [Read more](/hocuspocus/server/hooks#on-disconnect)              |
| `onListen`                 | When the server is initialized            | [Read more](/hocuspocus/server/hooks#on-listen)                  |
| `onDestroy`                | When the server will be destroyed         | [Read more](/hocuspocus/server/hooks#on-destroy)                 |
| `onConfigure`              | When the server has been configured       | [Read more](/hocuspocus/server/hooks#on-configure)               |
| `onRequest`                | When a HTTP request comes in              | [Read more](/hocuspocus/server/hooks#on-request)                 |
| `onStoreDocument`          | When a document has been changed          | [Read more](/hocuspocus/server/hooks#on-store-document)          |
| `onUpgrade`                | When the WebSocket connection is upgraded | [Read more](/hocuspocus/server/hooks#on-upgrade)                 |
| `onStateless`              | When the Stateless message is received    | [Read more](/hocuspocus/server/hooks#on-stateless)               |
| `beforeBroadcastStateless` | Before broadcast a stateless message      | [Read more](/hocuspocus/server/hooks#before-broadcast-stateless) |
| `afterUnloadDocument`      | When a document is closed                 | [Read more](/hocuspocus/server/hooks#after-unload-document)      |
...
The `onAwarenessUpdate` hooks are called when awareness changed ([Provider Awareness API](/hocuspocus/provider/events)).
...
The following example is not intended to be your primary storage as serializing to and deserializing from JSON will not store collaboration history steps but only the resulting document. This example is only meant to store the resulting document for the views of your application. For a primary storage, check out the [Database extension](/hocuspocus/server/extensions/database).
:::
...
The `onConfigure` hooks are called after the server was configured using the [configure method](/hocuspocus/server/methods). It should return a Promise.
...
The `onDestroy` hooks are called after the server was shut down using the [destroy](/hocuspocus/server/methods) method. It should return a Promise.
...
If you just want to get it working, have a look at the [`SQLite`](/hocuspocus/server/extensions/sqlite) extension for local development, and the generic [`Database`](/hocuspocus/server/extensions/database) extension for a convenient way to fetch and store documents.
