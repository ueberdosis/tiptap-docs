...
meta:
    title: Collaborative editing | Hocuspocus Docs
    category: Hocuspocus
---
...
const editor = new Editor({
  extensions: [
    StarterKit.configure({
      // The Collaboration extension comes with its own history/undoRedo handling
      undoRedo: false,
    }),
...
For most uses cases, a WebSocket provider is the recommended choice. It’s very flexible and can scale very well. To make it even easier, we released [Hocuspocus](/hocuspocus/introduction) as an open source backend for Tiptap.

Pick the client path that matches your app:

- **React apps** → use [`@hocuspocus/provider-react`](/hocuspocus/provider/react). It wraps the provider in components (`HocuspocusProviderWebsocketComponent`, `HocuspocusRoom`) and hooks so React handles the lifecycle for you — including StrictMode double-mounts.
- **Vanilla JS or other frameworks** → use the bare `HocuspocusProvider` directly and manage its lifecycle yourself.

#### React

Install the React bindings alongside the collaboration extension:

```bash
npm install @tiptap/extension-collaboration @hocuspocus/provider @hocuspocus/provider-react y-prosemirror yjs
```

Wrap your collaborative subtree in `HocuspocusProviderWebsocketComponent` + `HocuspocusRoom`, and read the provider from the `useHocuspocusProvider` hook inside your editor component:
...
The websocket component manages the shared socket. Multiple `HocuspocusRoom`s can share it — handy when a user has several documents open at once. See the [React bindings reference](/hocuspocus/provider/react) for connection/sync status, awareness, and event hooks.
...
And register the WebSocket provider with Tiptap directly:
...
Both examples above expect a WebSocket server at `ws://127.0.0.1:1234` — let's set that up next.

#### The WebSocket backend
To make the server part as easy as possible, we provide an opinionated server package, called [Hocuspocus](/hocuspocus/introduction). It’s a flexible Node.js package, that you can use to build your custom backend.
...
This command downloads the Hocuspocus command-line interface, starts a server listening on port 1234 and persists changes to a local SQLite file (drop the `--sqlite` flag to keep changes in memory only). The output should look like this:
...
The extension doesn't ship default styles. Drop these selectors into your stylesheet so remote carets, selections, and name labels are visible. The colors come from `user.color` via `currentColor`, so each user automatically gets their own tint:
