...
### Run Tiptap on the server

Improved SSR support with the ability to initialize the editor without rendering its content:
...
The `browserEnvironment` option unlocks full support for running Tiptap on the server, including running editor commands in headless environments like Node.js, Bun, or Deno. This enables server-side content processing.
...
Learn more about [running Tiptap on the server](/guides/run-tiptap-on-the-server).
