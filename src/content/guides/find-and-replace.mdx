...
## Combine matching options

The extension supports using Match case, Whole word, and Regex at the same time.
This behavior belongs to the headless extension and does not depend on React, Vue, or a particular
interface.

For example, this search enables all three options:
...
In `Catalog catalog cat cats`, it finds `catalog` and `cat`:

- `Catalog` is excluded because Match case is enabled.
- `cats` is excluded because Whole word is enabled.
- `cat|catalog` still matches the complete `catalog` alternative. The shorter `cat` alternative is
  not accepted inside the longer word.

Whole-word matching uses Unicode letters, combining marks, numbers, and connector punctuation such
as `_` as word characters. It also works for consecutive matches separated by punctuation.

## Reuse regex capture groups

When regex mode is enabled, replacement terms can reuse the complete match, numbered capture
groups, or named capture groups.
...
Replacement tokens are expanded only in regex mode. In literal mode, a replacement such as `$1`
inserts the literal characters `$1`. See the
[Find and Replace extension reference](/editor/extensions/functionality/find-and-replace#regex-replacement-tokens)
for the complete token list and unmatched-group behavior.
