...
Note that you need to replace `APP_ID` and `DOCUMENT_NAME` with your own values, and `YOUR_JWT` with a signed token carrying `Documents:Api:All` (see [Authentication](/authentication)). Keep that token server-side, since it grants full API access. The `APP_ID` is your document server ID, labeled "Document server ID" in the [Cloud dashboard](https://cloud.tiptap.dev/v2/configuration/document-server).
