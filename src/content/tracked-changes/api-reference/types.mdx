...
- `split` is a suggested [block split](/tracked-changes/usage/advanced-usage#block-splits). Accepting keeps the two blocks, rejecting merges them back together
...
Render your own label for splits (for example "Paragraph break") instead of `suggestion.text`.
