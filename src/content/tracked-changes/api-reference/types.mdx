...
- `split` is a suggested [block split](/tracked-changes/usage/advanced-usage#block-splits). Use `splitKind` to tell an end split from a middle split.

## SuggestionSplitKind

Describes how a `split` suggestion changed the document:

```ts
type SuggestionSplitKind = 'insertion' | 'structural'
```

- `insertion` is created when Enter adds a new block at the end of an existing block. Rejecting removes the new block.
- `structural` is created when Enter splits existing content. Rejecting merges the two blocks again.
...
Use `nodeSuggestionTypeToSuggestionType` to convert (see [Type helpers](#type-helpers)).
...
type Suggestion = {
  id: string
  type: SuggestionType
  // Only present for 'split' suggestions:
  splitKind?: SuggestionSplitKind
  userId: string
  createdAt: string
  updatedAt: string
  ...
  // Only present for 'attributeChange' and 'split' suggestions:
  nodeType?: string
...
- `splitKind` is `'insertion'` for an end split and `'structural'` for a middle split
- `nodeType` is the name of the affected block, such as `paragraph`, `heading`, or `listItem`
- `text` contains the current text in the affected block. Text typed after an end split is included in this field.
- `fullText` contains the complete current text in the affected block. It normally matches `text` for split suggestions.
- `from` and `to` span the block after the split

Use `splitKind` and `nodeType` to render a label such as `new paragraph` or `split heading`. Do not use a generic label that makes insertion and structural splits look the same.
...
Builds the marker element shown at a block split. Called once per marker, so it must return a new element every time:
...
- `blockSplitMarker` — the marker rendered at a suggested block split. Defaults to `'¶'`. Set it to `null` to render no marker. See [blockSplitMarker](/tracked-changes/getting-started/install#blocksplitmarker).
...
// Convert a node-level type to the public-facing suggestion type
