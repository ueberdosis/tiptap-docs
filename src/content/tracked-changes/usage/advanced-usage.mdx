...
When you press Enter in existing content, the extension tracks the new boundary without treating the second block as newly inserted. For example, splitting `<p>HelloWorld</p>` between `Hello` and `World` creates one structural split:
