...
When a user presses Enter inside a paragraph, heading, or table cell, only the block boundary is tracked. The text itself is left alone. Pressing Enter between the two sentences in `<p>First sentence.Second sentence.</p>` produces:

```html
<p>First sentence.</p>
<p data-suggestion data-suggestion-type="blockSplit">Second sentence.</p>
```

`Second sentence.` stays plain in the second block. There is no deletion copy in the first block, so the document stays readable while the split is pending. A `¶` marker is rendered at the end of the first block to show where the break was added. See [`blockSplitMarker`](/tracked-changes/getting-started/install#blocksplitmarker) to configure or remove it.

This matches how word processors show a tracked paragraph break.
...
Splitting a block that is already split creates a second, independent `split` suggestion. Typing inside a split-off block creates a normal `add` suggestion, separate from the split.

### `split` vs `blockSplit`

The same change is named differently depending on where you read it:

- `suggestion.type` from the query API is `'split'`
- the node attribute and the `data-suggestion-type` DOM attribute are `'blockSplit'`

Filter and switch on `'split'`. Write CSS against `'blockSplit'`. See [SuggestionNodeType](/tracked-changes/api-reference/types#suggestionnodetype) for the full mapping.

A `split` suggestion has an empty `text`, because no text was added or removed. Its `from` and `to` span the second block. Give splits their own label in review UIs instead of rendering `suggestion.text`, which would be blank.

### List items

Splitting a list item keeps the earlier behavior instead of creating a `split` suggestion:

- Pressing Enter at the **end** of an item marks the new `<li>` as an `add`
- Pressing Enter **mid-item** is tracked as a replacement, so rejecting restores the single item without losing text

A mid-item split is one suggestion with `type: 'replace'`. Filter on `'replace'`. Internally it is stored as a `replaceDeletion` / `replaceInsertion` mark pair, but those are mark-level values that never appear as `suggestion.type` — you only meet them in `data-suggestion-type` when writing CSS.

### Edge cases

Two of these involve deleted content, and which one applies depends on where the cursor is:

- Pressing Enter at the **very start** of a block whose whole content is pending deletion inserts a fresh empty block above it, marked `add`. This gives the user somewhere to type. Rejecting removes only that empty block, leaving the pending deletion alone. This covers both a block node marked `delete` and a block whose every text child carries a delete mark.
- Pressing Enter **anywhere else inside** deleted text is ignored. Content pending deletion cannot be restructured while the deletion is still open.
- Splitting inside an inserted wrapper (for example a suggested blockquote) stays covered by that outer suggestion instead of adding a nested split.
