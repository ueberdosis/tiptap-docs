...
When a user presses Enter inside a paragraph, heading, or table cell, only the paragraph boundary is tracked. The text itself is left alone. Pressing Enter between the two sentences in `<p>First sentence.Second sentence.</p>` produces:
...
- Pressing Enter **mid-item** is tracked as a `replaceDeletion` / `replaceInsertion` pair, so rejecting restores the single item without losing text

### Edge cases

- Splitting at the start of a block that is already pending deletion inserts a fresh empty block above it, marked `add`. Rejecting removes only that empty block.
...
- Content pending deletion cannot be split at all. The Enter press is ignored.
