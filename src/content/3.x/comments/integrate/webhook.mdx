...
- In conjunction with the [mention extension](/3.x/editor/extensions/nodes/mention), sending emails or notifications to users when they are mentioned in comments.
...
1. In case you’ve previously implemented Collaboration webhooks, check the `type` and `trigger` fields when processing incoming webhooks. ([Documentation](/3.x/collaboration/core-concepts/webhooks))
