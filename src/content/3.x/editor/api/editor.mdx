...
By default, Tiptap enables all [input rules](/3.x/editor/extensions/custom-extensions/extend-existing#input-rules). With `enableInputRules` you can control that.
...
By default, Tiptap enables all [paste rules](/3.x/editor/extensions/custom-extensions/extend-existing#paste-rules). With `enablePasteRules` you can control that.
...
Don’t confuse methods with [commands](/3.x/editor/api/commands). Commands are used to change the state of editor (content, selection, and so on) and only return `true` or `false`.
...
See the [NodePos class](/3.x/editor/api/node-positions).
