...
- **Creating and Toggling Lists:** Create or switch between list types using [`toggleList`](/3.x/editor/api/commands/lists/toggle-list).
- **Nesting and Unnesting List Items:** Lift or sink list items using commands like [`liftListItem`](/3.x/editor/api/commands/lists/lift-list-item) and [`sinkListItem`](/3.x/editor/api/commands/lists/sink-list-item).
- **Splitting and Wrapping List Items:** Split or wrap list items efficiently with [`splitListItem`](/3.x/editor/api/commands/lists/split-list-item) and [`wrapInList`](/3.x/editor/api/commands/lists/wrap-in-list).
- **Improving List Keyboard Behavior:** Use the [`List Keymap`](/3.x/editor/extensions/functionality/listkeymap) extension to refine list behavior with additional keymap handlers.

## List Keymap Extension

You might also want to include the [`List Keymap`](/3.x/editor/extensions/functionality/listkeymap) extension, which adds extra keymap handlers to change the default backspace and delete behavior for lists. It modifies the default behavior so that pressing backspace at the start of a list item lifts the content into the list item above.
