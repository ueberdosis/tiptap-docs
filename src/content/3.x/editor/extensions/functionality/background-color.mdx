...
This extension enables you to set the background color in the editor. It uses the [`TextStyle`](/3.x/editor/extensions/marks/text-style) mark, which renders a `<span>` tag (and only that). The background color is applied as inline style then, for example `<span style="background-color: #958DF1">`.
...
This extension requires the [`TextStyle`](/3.x/editor/extensions/marks/text-style) mark.
