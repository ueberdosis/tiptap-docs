...
Did you know that you can use [`getAttributes`](/3.x/editor/api/editor#getattributes) to find out which attributes, for example which href, is currently set? Don’t confuse it with a [command](/3.x/editor/api/commands) (which changes the state), it’s just a method. Here is how that could look like:
