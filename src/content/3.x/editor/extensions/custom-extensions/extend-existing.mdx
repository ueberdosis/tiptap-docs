...
The extension name is also part of the JSON. If you [store your content as JSON](/3.x/guides/output-json-html#option-1-json), you need to change the name there too.
...
Tiptap works with a strict schema, which configures how the content can be structured, nested, how it behaves and many more things. You [can change all aspects of the schema](/3.x/editor/core-concepts/schema) for existing extensions. Let’s walk through a few common use cases.
...
Take a closer look at [the full source code](https://github.com/ueberdosis/tiptap/tree/main/packages/extension-text-align) of the [`TextAlign`](/3.x/editor/extensions/functionality/textalign) extension to see a more complex example. But here is how it works in a nutshell:
...
The `parseHTML()` function tries to load the editor document from HTML. The function gets the HTML DOM element passed as a parameter, and is expected to return an object with attributes and their values. Here is a simplified example from the [`Bold`](/3.x/editor/extensions/marks/bold) mark:
