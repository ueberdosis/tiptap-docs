...
Nodes are the building blocks of your editor. They can be blocks or inline nodes. Good examples to learn from are [`Paragraph`](/3.x/editor/extensions/nodes/paragraph), [`Heading`](/3.x/editor/extensions/nodes/heading), or [`CodeBlock`](/3.x/editor/extensions/nodes/code-block).

They extend all of the options and methods from the [Extension API](/3.x/editor/extensions/custom-extensions/extension) and add a few more specific to nodes.
...
This will be used during copy events to render the mark as HTML. For more details, see the [extend existing extensions](/3.x/editor/extensions/custom-extensions/extend-existing#render-html) guide.
...
For more details, see the [extend existing extensions](/3.x/editor/extensions/custom-extensions/extend-existing#attributes) guide.
...
The content expression for this node, as described in the [schema guide](/3.x/docs/guide/#schema.content_expressions). When not given, the node does not allow any content.
...
There is a whole lot to learn about node views, so head over to the [dedicated section in our guide about node views](/3.x/editor/extensions/custom-extensions/node-views) for more information. If you are looking for a real-world example, look at the source code of the [`TaskItem`](/3.x/editor/extensions/nodes/task-item) node. This is using a node view to render the checkboxes.
