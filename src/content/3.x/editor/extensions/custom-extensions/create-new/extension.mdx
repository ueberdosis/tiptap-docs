...
If the extension is a node or a mark, the name is used to identify the node or mark in the editor's schema and therefore persisted to the JSON representation of the editor's content. See [store your content as JSON](/3.x/guides/output-json-html#option-1-json) for more information.
...
The [`Link`](/3.x/editor/extensions/marks/link) mark for example has a higher priority, which means it will be rendered as `<a href="…"><strong>Example</strong></a>` instead of `<strong><a href="…">Example</a></strong>`.
...
You can even move your [event listeners](/3.x/editor/api/events) to a separate extension. Here is an example with listeners for all events:
