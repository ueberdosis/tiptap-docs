...
Webhooks are not retried by default, but you can enable retries by setting `webhook_retries` to `1` (see [Configure Runtime](/3.x/collaboration/operations/configure)).
...
In order to initialize documents, you can use the `webhook_loader_url` setting (see [configure runtime](/3.x/collaboration/operations/configure)). This URL will be called if a new document is requested.
