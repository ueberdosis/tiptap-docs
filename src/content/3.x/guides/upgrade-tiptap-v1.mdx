...
meta:
  title: Upgrade v1 to v2 | Tiptap Collaboration Docs
  description: Upgrade your Tiptap v1 project to version 2. Learn about the changes and how to migrate your project.
...
Tiptap 1 tried to hide a few required extensions from you with the default setting `useBuiltInExtensions: true`. That setting has been removed and you’re required to import all extensions. Be sure to explicitly import at least the [`Document`](/3.x/editor/extensions/nodes/document), [`Paragraph`](/3.x/editor/extensions/nodes/paragraph) and [`Text`](/3.x/editor/extensions/nodes/text) extensions.
...
And we removed some settings: `dropCursor`, `enableDropCursor`, and `enableGapCursor`. Those are separate extensions now: [`Dropcursor`](/3.x/editor/extensions/functionality/dropcursor) and [`Gapcursor`](/3.x/editor/extensions/functionality/gapcursor). You probably want to load them, but if you don’t, just ignore this.
...
Read more about [all the nifty details building custom extensions](/3.x/editor/extensions/custom-extensions) in our guide.

## Renamed settings and methods

[We renamed a lot of settings and methods](/3.x/editor/api/editor). Hopefully you can migrate to the new API with search & replace. Here is a list of what changed:
...
Read the dedicated [guide on creating menus](/3.x/editor/getting-started/style-editor/custom-menus) to migrate your menus.
...
The `.chain()` is required to start a new chain and the `.run()` is needed to actually execute all the commands in between. Read more about [the new Tiptap commands](/3.x/editor/api/commands) in our API documentation.
...
The new event callbacks have fewer parameters. The same things should be available through `this.` now. [Read more about events here.](/3.x/editor/api/events)
...
Read more about [the new collaborative editing experience](/3.x/collaboration/getting-started/install) in our guide.
