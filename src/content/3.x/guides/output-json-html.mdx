...
Our editor has top notch support for Y.js, which is amazing to add features like [realtime collaboration, offline editing, or syncing between devices](/3.x/collaboration/getting-started/overview).
...
### Markdown (Not an option)

**Tiptap doesn’t support Markdown as an input format**. We decided against supporting Markdown because:
...
- There are packages to convert HTML to Markdown and vice-versa.

You probably already considered to work with HTML or JSON to store your content but probably have to rely on it anyway.

If you need Markdown, ProseMirror has an [example on how to deal with Markdown](https://prosemirror.net/examples/markdown/) and if you are looking for a really good Markdown editor, try [CodeMirror](https://codemirror.net/).

That said, Tiptap does support [Markdown shortcuts](/3.x/examples/basics/markdown-shortcuts) to format your content. Also you’re free to let your content look like Markdown, for example add a `#` before an `<h1>` with CSS.

If you are looking to convert ProseMirror JSON to Markdown, you can use the `@tiptap/static-renderer` package, which can convert ProseMirror JSON to Markdown. See more info and examples in it's [documentation](/3.x/editor/api/utilities/static-renderer).

## Listen for changes

If you want to continuously store the updated content while people write, you can [hook into events](/3.x/editor/api/events). Here is an example how that could look like:
...
To learn more about the static renderer, check out the [static renderer documentation](/3.x/editor/api/utilities/static-renderer).
