...
<Callout title="Custom LLM demo" variant="info">
  While our custom LLM integration is available to all paid plans, we provide our Business and
  Enterprise customers with a detailed demo. It includes the client and server code, and
  instructions on how to run and deploy it. See [plans and pricing](https://tiptap.dev/pricing).
</Callout>
