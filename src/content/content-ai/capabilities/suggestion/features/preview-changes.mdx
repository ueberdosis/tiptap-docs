...
meta:
  title: Preview changes | Tiptap AI Suggestion
...
The AI Suggestion extension can be used to preview changes before they are applied to the document.

This feature displays changes as a diff that compares the old and new documents, and enables users to approve and reject individual changes.

<Callout title="Note" variant="info">
  To review changes **after** they are applied, integrate the [AI
  Changes](/content-ai/capabilities/changes) extension instead.
</Callout>

<Callout title="Beta feature" variant="info">
  This API is experimental and might change in the future. We appreciate your feedback! [Contact
  us](humans@tiptap.dev).
</Callout>
  ...
  extensions: [
    ...
    AiSuggestion.configure({
      ...
      rules: [
        {
          id: 'diff',
          // No need to define a title, prompt, and colors for diff rules
          title: '',
          prompt: '',
          color: '#000',
          backgroundColor: '#fff',
...
Generate suggestions by passing the new content of the document to the resolver function (in html format). The AI Suggestion extension will compare the new content with the current content and generate a list of suggestions that belong to the `'diff'` rule.
