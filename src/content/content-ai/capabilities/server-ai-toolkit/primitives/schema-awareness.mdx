...
After calling this method, the Server AI Toolkit will use the new schema. The Server AI Toolkit's current document will also be reset to an empty document, because the previous document is no longer valid for the new schema.be validated against the new schema.
