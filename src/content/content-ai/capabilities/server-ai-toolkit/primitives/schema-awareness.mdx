...
The schema awareness data is serializable, meaning that it can be encoded as a JSON string and sent over the network or stored in a database.
...
Alternatively, you can store the schema awareness data in the server, in your database or file system.
...
If you call this method, the Server AI Toolkit will use the new schema. The Server AI Toolkit's current document might not be valid for the new schema, so you might need to switch the current document, too.
