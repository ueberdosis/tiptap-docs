...
Create a session before you call any AI Toolkit tools. The session API returns a `sessionId` that you reuse with subsequent `tiptapRead` and `tiptapEdit` requests.
...
Use the same `sessionId` in later tool calls so the Server AI Toolkit can validate that the content has not changed between a read and a follow-up edit.
