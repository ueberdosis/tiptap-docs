...
export async function handleCollaborationTokenRequest(request: Request): Promise<Response> {
  const { documentId, userId }: { documentId: string; userId: string } = await request.json()

  if (!TIPTAP_PRIVATE_KEY || !TIPTAP_ENVIRONMENT_ID) {
    throw new Error(
      'TIPTAP_PRIVATE_KEY and TIPTAP_ENVIRONMENT_ID environment variables must be set',
    )
  }
