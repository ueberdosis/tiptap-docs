...
const apiBaseUrl = 'https://api.tiptap.dev/v4/ai'
...
export async function POST(req: Request) {
  ...
  const { tools, systemPrompt } = await fetch(`${apiBaseUrl}/toolkit/fetch-tools`, {
    method: 'POST',
  ...
  const upstream = streamToolFetch(`${apiBaseUrl}/toolkit/stream-tool`, {
    method: 'POST',
