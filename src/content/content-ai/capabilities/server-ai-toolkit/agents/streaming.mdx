...
<Callout title="Requires the new authentication" variant="info">
  `stream-tool` requires the new [environment authentication](/authentication): an ES256 token for
  the `AI` audience with the `AI:Toolkit` permission. Because it edits a Tiptap Cloud document, the
  same token also needs the `Documents` audience and a `Documents:Write` permission.
</Callout>
