...
2. The AI edits the document with `tiptapEdit` in `trackedChanges` mode — the server writes tracked change marks instead of directly mutating the document.
3. If the edit operations include `meta`, the server turns that metadata into comment threads through the Comments API.
4. The client displays the tracked changes and lets users accept or reject them.

## Show tracked changes

To display server-side AI edits as tracked changes, pass `reviewOptions` with mode `'trackedChanges'` when executing tools.
