...
To display server-side AI edits as tracked changes, pass `reviewOptions` with mode `'trackedChanges'` and a `sessionId` when executing tools.
...
When fetching tool definitions, pass the `operationMeta` option to add a `meta` field to edit operations. Then instruct the AI to provide a justification in the `meta` field.
