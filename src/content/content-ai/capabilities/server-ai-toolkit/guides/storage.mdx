...
```typescript
import { ServerAiToolkitStorage } from '@tiptap-pro/ai-toolkit/server-ai-toolkit'
...
class RedisStorage implements ServerAiToolkitStorage {
  private client: ReturnType<typeof createClient>

  constructor(private documentKey: string) {
    this.client = createClient()
    this.client.connect()
  }

  async getDocument() {
    const content = await this.client.get(this.documentKey)
    return JSON.parse(content || '{}')
  }

  async setDocument(document) {
    await this.client.set(this.documentKey, JSON.stringify(document))
  }
}
...
const toolkit = getServerAiToolkit({
  schemaAwarenessData,
  storage: new RedisStorage('doc:123'),
})
```
...
- Learn about [document persistence methods](/content-ai/capabilities/server-ai-toolkit/primitives/storage-and-document-persistence)
