...
<Requirements>
  <RequirementItem label="1. Install the package">
    The Server AI Toolkit package is open source. Install it from the public npm registry using npm
    or your preferred package manager.
  </RequirementItem>
  <RequirementItem label="2. Add the extension">
    Add the `ServerAiToolkit` extension to your editor so the server can understand your document
    schema.
  </RequirementItem>
  <RequirementItem label="3. Authenticate API requests">
    Configure your Content AI credentials to access the Server AI Toolkit cloud service, or use your
...
Work with Tiptap documents on the server. The Server AI Toolkit includes both tools for AI agents
and workflows for single-purpose automations.
...
- [**Workflows**](/content-ai/capabilities/server-ai-toolkit/workflows) where the AI has a single,
  well-defined task: insert content, proofread the document, and more.
...
Build focused server-side AI automations that follow a fixed read → generate → execute flow.
