...
The Server AI Toolkit is a cloud or on-premises service. Authenticate its REST API with a signed token carrying `aud: "AI"` and the `AI:Toolkit` permission. When the toolkit should fetch and save your Tiptap Cloud documents automatically, also grant `Documents:Write`. See [Authentication](/authentication) for how to create a key pair and sign tokens.
...
This function signs a token for the Server AI Toolkit API. Pass a `documentId` when the toolkit should fetch and save Tiptap Cloud documents automatically via `experimental_documentOptions`.
...
This function returns the headers required for Server AI Toolkit API requests. No App ID is sent.
