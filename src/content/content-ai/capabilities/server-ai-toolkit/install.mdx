...
The `createJwtToken` function signs a token for the Server AI Toolkit API. Pass a `documentId` when the toolkit should fetch and save Tiptap Cloud documents automatically via `experimental_documentOptions`.
...
<Callout title="Maintaining an existing integration?" variant="info">
  ...
  document server credentials as JWT claims, is documented under [Legacy authentication](/authentication/legacy).
</Callout>
