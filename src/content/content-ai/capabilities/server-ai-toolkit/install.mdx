...
1. **Create a secret** in your Tiptap dashboard. The dashboard generates an ES256 key pair and shows you the **private key** once. Store that private key on your server.
2. **Copy your environment hash ID** from the dashboard. Use it as the JWT `iss` claim.
3. **Generate a Tiptap Access Control JWT** on your server. When building the JWT, always include the `AI` audience and `AI:Toolkit` permission. Include the `Documents` audience and the `Documents:Write` permission when the toolkit should read or write Tiptap Cloud documents.
4. **Use the JWT** in API requests. Pass it in the `Authorization` header as a Bearer token.

For the full JWT format and signing examples, see the [Tiptap Access Control guide](/content-ai/capabilities/server-ai-toolkit/tiptap-access-control) and the [Authentication reference](/authentication).
...
- **TIPTAP_ENVIRONMENT_ID**: Your environment hash ID from the Tiptap dashboard. Used as the JWT `iss` claim.
- **TIPTAP_PRIVATE_KEY**: Your ES256 private key (PKCS#8 PEM) from the dashboard secret. Used to sign JWTs server-side.

### Create a JWT token

Create the JWT token to authenticate to the Server AI Toolkit. Follow the [Tiptap Access Control guide](/content-ai/capabilities/server-ai-toolkit/tiptap-access-control#set-up-authorization-with-tiptap-access-control) to create the token. Pass a `documentId` when the Server AI Toolkit should fetch and save a Tiptap Cloud document.

Set the audience and permissions in the JWT:

```ts
const audience = ['AI']
const permissions = [{ action: 'AI:Toolkit', resource: '*' }]

// When using the Server AI Toolkit to read and edit Tiptap Cloud
// documents, configure access to them in the JWT.
if (documentId) {
...
Create a `createJwtToken` function that creates the JWT token.
