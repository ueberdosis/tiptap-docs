...
2. **Generate a JWT** using a library like `jsonwebtoken`. In production, always create JWTs **server-side** to keep your secret safe.
...
```ts
const response = await fetch(`${TIPTAP_CLOUD_AI_API_URL}/v3/toolkit/tools`, {
  method: 'POST',
  headers,
  body: JSON.stringify({
    schemaAwarenessData,
