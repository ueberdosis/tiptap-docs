...
1. **Create an ES256 signing key** in your Tiptap dashboard.
2. **Generate a JWT** using the private key. Include `AI:Toolkit` permission and, when the toolkit should read or write Tiptap Cloud documents, `Documents:Write` for the target document.
3. **Use the JWT** in your API requests. Pass it in the `Authorization` header as a Bearer token.
...
- **TIPTAP_ENVIRONMENT_ID**: Your environment hash ID from the Tiptap dashboard. Used as the `iss` claim.
...
This function returns the authentication headers required for Server AI Toolkit API requests.
