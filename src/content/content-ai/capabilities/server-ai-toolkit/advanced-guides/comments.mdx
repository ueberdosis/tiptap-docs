...
const response = await fetch(`${apiBaseUrl}/toolkit/tools`, {
  ...
  body: JSON.stringify({
    schemaAwarenessData,
    tools: {
      // tiptapRead and tiptapEdit are enabled by default but you can disable them
      // so that the AI can only comment and not update the document.
      tiptapRead: false,
      tiptapEdit: false,
      // Enable comment tools
      getThreads: true,
...
- `documentId` (`string`, required): The ID of the document to create comments in.
- `apiSecret` (`string`, required): The API secret for the Server AI Toolkit.
- `userId` (`string`, required): The ID of the user creating comments.
