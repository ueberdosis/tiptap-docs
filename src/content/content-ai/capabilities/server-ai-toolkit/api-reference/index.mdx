---
title: API Reference
meta:
  title: API Reference | Tiptap Server AI Toolkit
  description: Complete API reference for the Server AI Toolkit.
  category: Content AI
---

import { Link } from '@/components/Link'

The Server AI Toolkit provides utilities and a REST API for working with Tiptap documents on the server.

<div className="grid md:grid-cols-2 gap-4 mt-6 md:items-start">
  <Link
    href="/content-ai/capabilities/server-ai-toolkit/api-reference/workflows"
    className="flex flex-col p-4 border border-grayAlpha-200 rounded-lg hover:border-grayAlpha-300 transition-colors cursor-pointer no-underline"
  >
    <div className="flex items-center justify-between mb-2">
      <h4 className="font-semibold text-black">Workflows</h4>
      <span className="text-sm text-blue-600 font-medium whitespace-nowrap ml-4">More →</span>
    </div>
    <div className="text-sm text-grayAlpha-700">
      Reference for workflow definition, workflow read, and workflow execute endpoints.
    </div>
  </Link>
  <Link
    href="/content-ai/capabilities/server-ai-toolkit/api-reference/schema-awareness"
    className="flex flex-col p-4 border border-grayAlpha-200 rounded-lg hover:border-grayAlpha-300 transition-colors cursor-pointer no-underline"
  >
    <div className="flex items-center justify-between mb-2">
      <h4 className="font-semibold text-black">Schema awareness</h4>
      <span className="text-sm text-blue-600 font-medium whitespace-nowrap ml-4">More →</span>
    </div>
    <div className="text-sm text-grayAlpha-700">
      Give your AI the ability to understand the document and generate custom elements.
    </div>
  </Link>
  <Link
    href="/content-ai/capabilities/server-ai-toolkit/api-reference/review-options"
    className="flex flex-col p-4 border border-grayAlpha-200 rounded-lg hover:border-grayAlpha-300 transition-colors cursor-pointer no-underline"
  >
    <div className="flex items-center justify-between mb-2">
      <h4 className="font-semibold text-black">Review options</h4>
      <span className="text-sm text-blue-600 font-medium whitespace-nowrap ml-4">More →</span>
    </div>
    <div className="text-sm text-grayAlpha-700">
      Configure direct edits and tracked changes for server-side tools and workflows.
    </div>
  </Link>
  <Link
    href="/content-ai/capabilities/server-ai-toolkit/api-reference/rest-api"
    className="flex flex-col p-4 border border-grayAlpha-200 rounded-lg hover:border-grayAlpha-300 transition-colors cursor-pointer no-underline"
  >
    <div className="flex items-center justify-between mb-2">
      <h4 className="font-semibold text-black">REST API</h4>
      <span className="text-sm text-blue-600 font-medium whitespace-nowrap ml-4">More →</span>
    </div>
    <div className="text-sm text-grayAlpha-700">
      Complete reference for the Server AI Toolkit REST API endpoints.
    </div>
  </Link>
</div>
