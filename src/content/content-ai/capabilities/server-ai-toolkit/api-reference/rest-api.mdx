...
The Server AI Toolkit provides REST API endpoints for creating sessions, getting tool definitions, and executing tools on Tiptap documents.
...
Create a session before calling `tiptapRead` and `tiptapEdit`. The Server AI Toolkit stores the last read snapshot in that session and uses it to reject stale edits.

### Endpoint

```
POST /v3/ai/toolkit/create-session
```

### Request

```curl
curl --location 'BASE_URL/v3/ai/toolkit/create-session' \
...
- `sessionId` (`string`): Session identifier to reuse with later `tiptapRead` and `tiptapEdit` calls

### Example response

```json
{
  "sessionId": "6d7f14a7-ec5d-4a84-8bfc-5e6fcdaf0b2b"
}
```

<Callout title="Sessions are process-local" variant="warning">
  Sessions are stored in memory by the current Server AI Toolkit process. Reuse the returned
  `sessionId` against the same deployment instance, and create a new session if your infrastructure
  restarts.
</Callout>
...
- `operationMeta?` (`string | null`, optional): Adds a `meta` string field to `tiptapEdit` operations. Use this when you want the AI to attach a justification or other per-operation metadata.
...
<Callout title="Session required" variant="info">
  `sessionId` is required for every `execute-tool` request. Call `create-session` first, reuse the
  returned ID for `tiptapRead` and the follow-up `tiptapEdit`, and create a new session when you
  start a new read/edit cycle.
</Callout>
...
- `sessionId` (`string`, required): Session identifier returned by `create-session`. The server uses it to store the last `tiptapRead` snapshot and validate follow-up edits.
...
- `reviewOptions?` ([`ReviewOptions`](/content-ai/capabilities/server-ai-toolkit/api-reference/review-options), optional): Review-mode configuration for tool execution. See the [review options reference](/content-ai/capabilities/server-ai-toolkit/api-reference/review-options).
- `format?` (`string`, optional): The format in which the AI reads and edits the document. Must be one of:
  - `"json"` (default): Standard JSON format
  - `"shorthand"`: Tiptap Shorthand format, which reduces AI token costs by up to 80% compared to standard JSON
