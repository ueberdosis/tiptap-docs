...
<Callout title="Postman collection" variant="info">Browse the <a href="https://www.postman.com/docking-module-explorer-14290287/tiptap-collaboration-public/collection/33042171-acccd349-873c-41cc-b8b5-3b9e389cb029?action=share&source=copy-link&creator=42722929" target="_blank" rel="noopener noreferrer">Postman collection</a> for the Server AI Toolkit REST API.</Callout>
...
```curl
curl --location 'BASE_URL/v3/toolkit/tools' \
...
```curl
curl --location 'BASE_URL/v3/toolkit/execute-tool' \
--header 'Content-Type: application/json' \
--header 'Authorization: Bearer YOUR_JWT_TOKEN' \
--header 'X-App-Id: YOUR_APP_ID' \
--data '{
...
```curl
curl --location 'BASE_URL/v3/toolkit/schema-awareness-prompt' \
--header 'Content-Type: application/json' \
--header 'Authorization: Bearer YOUR_JWT_TOKEN' \
--header 'X-App-Id: YOUR_APP_ID' \
--data '{
