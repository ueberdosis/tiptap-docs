...
<Callout title="Postman collection" variant="info">A <a href="https://www.postman.com/docking-module-explorer-14290287/tiptap-collaboration-public/collection/33042171-acccd349-873c-41cc-b8b5-3b9e389cb029?action=share&source=copy-link&creator=42722929" target="_blank" rel="noopener noreferrer">Postman collection</a> for the Server AI Toolkit API is available.</Callout>

## Base URL

In the examples, `API_BASE_URL` is the base URL of the Tiptap Cloud AI service. The base URL of the Tiptap Cloud AI service is `https://api.tiptap.dev`. For on-premises deployments, this URL will be different.
...
```
POST /v3/toolkit/tools
```

### Request

```curl
curl --location 'API_BASE_URL/v3/toolkit/tools' \
...
```
POST /v3/toolkit/execute-tool
```

### Request

```curl
curl --location 'API_BASE_URL/v3/toolkit/execute-tool' \
--header 'Content-Type: application/json' \
--header 'Authorization: Bearer YOUR_JWT_TOKEN' \
--header 'X-App-Id: YOUR_APP_ID' \
--data '{
...
```
POST /v3/toolkit/schema-awareness-prompt
```

### Request

```curl
curl --location 'API_BASE_URL/v3/toolkit/schema-awareness-prompt' \
--header 'Content-Type: application/json' \
--header 'Authorization: Bearer YOUR_JWT_TOKEN' \
--header 'X-App-Id: YOUR_APP_ID' \
--data '{
