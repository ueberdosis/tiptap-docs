...
<Callout title="Backward compatibility" variant="warning">
  ...
  `/v3/ai/toolkit/get-tools` for new integrations.
</Callout>
...
- `editorContext` (`EditorContext`, required)
- `tools?` (`object`, optional): Enable or disable individual tools
- `toolConfig?` (`object`, optional): Configuration object for each tool
...
- `editorContext` (`EditorContext`, required)
- `toolConfig?` (`unknown`, optional): Tool-specific parameters provided by the developer
- `format?` (`'json' | 'shorthand'`, optional, default: `'json'`)
...
<Callout title="Workflow endpoints" variant="info">
...
- [Editor context](/content-ai/capabilities/server-ai-toolkit/api-reference/schema-awareness)
