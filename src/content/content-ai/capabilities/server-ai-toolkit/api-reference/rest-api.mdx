...
Streams tool execution over a duplex NDJSON connection: you send the AI-generated tool input incrementally as the model produces it, and the endpoint applies it and streams back its progress. For `tiptapEdit`, the edit appears in the document in real time. See the [Streaming guide](/content-ai/capabilities/server-ai-toolkit/agents/streaming) for an end-to-end example.
