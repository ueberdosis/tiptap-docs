...
All requests must include a signed token in the `Authorization` header:

- `Authorization: Bearer <JWT>`

Sign the token server-side with `aud: "AI"` and the `AI:Toolkit` permission. To fetch and save Tiptap Cloud documents automatically, also grant `Documents:Write`. See [Authentication](/authentication) for how to create a key pair and sign tokens, and the [authorization guide](/content-ai/capabilities/server-ai-toolkit/install#set-up-authorization) for the full setup.
...
The previous App ID and secret flow is documented under [Legacy authentication](/authentication/legacy).

### Document access

To let the Server AI Toolkit fetch and save Tiptap Cloud documents, include `Documents:Write` in the token's permissions and reference the document via `experimental_documentOptions` in the request body (see [How to provide the document](#how-to-provide-the-document)).
