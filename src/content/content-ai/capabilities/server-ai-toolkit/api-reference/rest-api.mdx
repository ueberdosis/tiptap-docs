...
Which authentication each endpoint accepts:

| Endpoints | Accepted authentication |
| --- | --- |
| All v4 endpoints | New authentication |
| v3 `tools`, `execute-tool` | Legacy or new authentication |
| v3 `stream-tool` | New authentication |

Note that `stream-tool` requires the new authentication on both v3 and v4, even though the other v3 endpoints still accept legacy tokens during migration.
