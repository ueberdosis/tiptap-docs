...
When inserting block content into the editor, sometimes you may want to insert it as a sibling instead of as a block directly. You can use the `respondInline` option to insert the content at the same depth as the `from` position.
...
// Output: <p>ABChello world</p><p>ok</p>
// As opposed to: <p>ABC</p><p>hello work</p><p>ok</p> when `respondInline` is not set
```
