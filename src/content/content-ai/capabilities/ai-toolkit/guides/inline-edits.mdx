...
Next, add an `editSelection` function to call the AI model when the button is clicked.

Inside this function, get the current selection with the `getHtmlSelection` method of the AI Toolkit. Then, call the API endpoint with the user's request and the selection. The API endpoint will return a stream of HTML that we can insert into the editor with the `streamHtml` method of the AI Toolkit.
