...
After the AI agent has finished its work, you can show a summary of the changes to the user.

In the [Review changes guide](/content-ai/capabilities/ai-toolkit/guides/review-changes), we built an AI agent chatbot that previews the changes before they are applied, every time it wants to edit the document. However, in this guide, the AI agent chatbot can perform as many changes as it wants, and then the user reviews all of them together.
...
When the AI modifies the document, the changes will be displayed as [suggestions](/content-ai/capabilities/ai-toolkit/primitives/display-suggestions) in the editor.
...
To accept all changes, just hide the diff view and stop the real-time document comparison.
...
To reject all changes, call the `rejectAllChanges` method. This will reset the editor content to how it was before the AI made changes.
