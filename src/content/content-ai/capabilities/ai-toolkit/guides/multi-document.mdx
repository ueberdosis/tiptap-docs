...
This component will use the [useChat hook](https://ai-sdk.dev/docs/reference/ai-sdk-ui/use-chat) from the Vercel AI SDK to call the API endpoint and manage the chat conversation. When the AI model outputs a tool call, it is executed in one of two ways:
...
The multi-document system we created in this guide is very simple, because it is stored in memory and not persisted. You can persist the documents in your database, in a file system, or save them as collaborative documents in [Tiptap Cloud](/collaboration/getting-started/overview).

Besides that, you can also make the document structure more sophisticated by, for example, allowing the AI to create folders to organize the documents.

Besides that, we recommend combining the multi-document system with the following AI Toolkit capabilities:
