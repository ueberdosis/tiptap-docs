...
To insert content into the document without streaming it, use the `insertHtml` method. This method has the same options as `streamHtml`, only that it is called once with the full HTML content as argument. [See the API reference](/content-ai/capabilities/ai-toolkit/api-reference/edit-the-document#inserthtml).
...
<Callout title="Work in progress" variant="info">
  We are working on integrating the Insert Content workflow with Tracked Changes, this feature will
