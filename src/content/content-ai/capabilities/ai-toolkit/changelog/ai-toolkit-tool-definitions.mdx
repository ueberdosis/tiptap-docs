...
- Simplify tool definitions of the tiptapRead and tiptapEdit tools
...
- Fix the published CLI so `tool-definitions` and the workflow commands run correctly when invoked as a package binary.
...
- Requires `@tiptap-pro/ai-toolkit` package version `3.0.0-alpha.88` or later.
- Rename `hash` to `target` in the proofreader workflow, for consistency with edit operations
...
- Add `requiredSlots`, `requiredConditions`, and `requiredAttributes` options to `createTemplateWorkflow`. When omitted, all fields are required (the default). Pass an empty array to make all fields of a type optional.
...
- Add `createTemplateWorkflow` server-side workflow for generating template-filling prompts and schemas.
...
- Clarify `getThreads` pagination range to use an exclusive upper bound in tool output.

## 3.0.0-alpha.25

### Patch Changes

- Add pagination fields to the `getThreads` tool schema and description.
