...
- **Breaking**: Add new `smart` diff mode and set it as the default. The `smart` mode uses tree-aware A\* pathfinding to align block nodes optimally, then uses character-level diff for inline content within matched blocks and recursive tree diff for nested block content. Changes never cut across block boundaries, producing clean diffs for structural document changes.
- **Breaking**: Rename `detailed` diff mode to `inline`. The `detailed` mode name is no longer accepted; use `inline` instead.
- **Breaking**: Change the default diff mode from `detailed` to `smart`.
- **Breaking**: Replace `diffMode` property in `ReviewOptions` with `useDiffUtility` (`boolean`). Instead of `diffMode: 'detailed'`, use `useDiffUtility: true`. Instead of `diffMode: 'full'`, use `useDiffUtility: false`.
- **Breaking**: Change the default `changeMergeDistance` from `null` to `8`. Unchanged ranges shorter than 8 positions are now absorbed into adjacent changes by default. Set `changeMergeDistance: null` to restore the previous behavior.

### Deprecations

- Deprecate `smartInline` and `smartInlineV2` diff modes. Use `smart` mode instead. These modes will be removed in a future version.

### Minor Changes

- Improve performance of the inline diff mode by avoiding redundant tokenization with a token cache.
- Simplify Myers diff implementation and improve change merging logic.
