...
- Fix bug in `setHtmlSuggestions` and `setMarkdownSuggestions` where the `changes` format would cause dangerous substring replacements. Changes where insert contains delete as substring are now skipped to prevent corrupting existing words (e.g., `{delete: 'mo', insert: 'months'}` would turn 'morning' into 'monthsrning').
