...
- **Breaking**: Remove `getText`, `getHtml`, `getJson`, `getTextSelection`, `getHtmlSelection`, and `getJsonSelection` methods. Use the Tiptap Editor API or `getTextRange` instead.
...
- **Breaking**: Remove `setHtmlSuggestions` method. Use `setMarkdownSuggestions` instead.
- **Breaking**: Remove `smartInline` and `smartInlineV2` diff modes. Use `smart` mode instead.
