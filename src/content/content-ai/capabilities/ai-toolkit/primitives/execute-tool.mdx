...
Once you've added [tool definitions](/content-ai/capabilities/ai-toolkit/agents/tools) to your AI agent, the AI agent will generate tool calls. Use the `executeTool` method to apply the tool calls to the editor.
...
- `options` (`ExecuteToolOptions`): Configuration for tool execution
  - `toolName` (`string`): Tool to execute. Can be one of the [available tools](/content-ai/capabilities/ai-toolkit/agents/tools/available-tools).
...
For a complete hands-on tutorial on tool streaming, see the [AI agent chatbot with tool streaming guide](/content-ai/capabilities/ai-toolkit/agents/tool-streaming).
