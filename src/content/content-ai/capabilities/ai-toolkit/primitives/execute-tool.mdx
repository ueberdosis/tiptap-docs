...
Streams a tool call into the editor, executing it and applying edits incrementally as the tool call is streaming.

This method has the same effect as calling the `executeTool` method, but it lets you edit the document incrementally while the tool call is streaming, instead of editing the document all at once when the tool call has finished streaming.
...
const result = toolkit.streamTool({
  ...
  toolName: 'insertContent',
  input: {
    html: '<p>Streaming content</p>',
    position: 'documentEnd',
  },
  currentChunk: 0,
  hasFinished: false, // Tool is still streaming
})
...
const finalResult = toolkit.streamTool({
  ...
  toolName: 'insertContent',
  input: {
    html: '<p>Complete content</p>',
    position: 'documentEnd',
  },
  currentChunk: 0,
  hasFinished: true, // Tool streaming is complete
})
```
