...
Compare two documents in real time and show the diff as suggestions in the editor.
...
The `otherDoc` parameter is a [ProseMirror Node object](https://prosemirror.net/docs/ref/#model.Node). If it is omitted, the current document is used. Then, when the document changes, a diff of the document before and after the changes is displayed.
...
You can apply a suggestion to the current document. This is equivalent to rejecting a change.

```ts
const suggestions = toolkit.getSuggestions()

// Apply the first suggestion to the current document
toolkit.applySuggestion(suggestions[0].id)
```

To apply a suggestion to the other document, call the `applySuggestionToOtherDoc` method. This is equivalent to accepting a change.
...
To stop comparing documents, use the `stopComparingDocuments` method. This will hide the diff view and clear the suggestions.
...
Try clicking the "Start comparing documents" button, a diff will appear. Then, try editing the document and see the diff change.
