...
### Step 2 (optional): configure custom elements

If your document contains custom nodes or marks, you can configure them with the `customNodes` option. This way, the AI model will be able to generate these custom elements accurately. For example, if you have a custom node called `alert`, you can allow the AI model to generate it by configuring it like this:
...
const schemaAwareness = toolkit.getSchemaAwareness({
  customNodes: [
    {
      // The extension name of the custom node
      extensionName: 'alert',
...
The code below shows an API endpoint built with Next.js and the Vercel AI SDK, like in the [AI Agent chatbot guide](/content-ai/capabilities/ai-toolkit/guides/ai-agent-chatbot).
