...
Creates a ready-made workflow configuration for filling templates. Accepts an HTML template string (generated by `createHtmlTemplate` on the client), auto-extracts all template keys, and returns a system prompt and schemas for validating AI output.
...
- `options` (`CreateTemplateWorkflowOptions`): Configuration options
  - `htmlTemplate` (`string`): The template as an HTML string, generated by `createHtmlTemplate` on the client
...
// Get the HTML template and task from the API endpoint request
...
- `template` (`Record<string, any>`): The template as Tiptap JSON (a doc node or fragment array)
- `schema` (`Schema`): The editor's ProseMirror schema
...
```ts
import { createHtmlTemplate } from '@tiptap-pro/ai-toolkit'

const htmlTemplate = createHtmlTemplate(templateJson, editor.schema)
```
