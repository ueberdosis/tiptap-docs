...
- `simplifyChanges?` (`boolean`): Whether to simplify the changes so that if there are multiple changes in the same word, they are merged into one change. Only applies when mode is 'detailed', 'smartInline', or 'smartInlineV2'. Default: `true`
...
- `mode?` (`'detailed' | 'block' | 'smartInline' | 'smartInlineV2'`): The diff mode to use for comparison.
  ...
  - `'smartInline'` combines both approaches: first identifies changed blocks, then performs inline diff within those blocks. This is more efficient for documents where only portions have changed, while still providing character-level precision.
  - `'smartInlineV2'` extends this behavior by falling back to block-level changes when inline change slices include top-level block content.
