...
- `simplifyChanges?` (`boolean`): Whether to simplify the changes so that if there are multiple changes in the same word, they are merged into one change. Only applies when mode is 'detailed' or 'smartInline'. Default: `true`
