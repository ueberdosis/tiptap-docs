...
For complex agentic AI assistants like the one in the [AI agent chatbot](/content-ai/capabilities/ai-toolkit/guides/ai-agent-chatbot) guide, it is recommended to use a frontier model with advanced tool calling capabilities. In particular, the following models have shown positive results:
...
This way, the AI model doesn't need to call the `readNodeRange` tool to read the document, and can jump straight to editing.
