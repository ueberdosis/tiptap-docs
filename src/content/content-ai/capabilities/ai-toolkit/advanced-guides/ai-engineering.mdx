...
- AI agents are given a task and work independently towards it, making decisions about what next action to take at each step. For example: an [AI chatbot assistant](/content-ai/capabilities/ai-toolkit/agents/ai-agent-chatbot)
- In AI workflows, the AI performs one or more pre-defined actions, but does not get to choose the next action to take. This is the case in [simple AI content generation](/content-ai/capabilities/ai-toolkit/workflows/insert-content) or autocompletion.

### Best models for agentic tasks

For complex agentic AI assistants like the one in the [AI agent chatbot](/content-ai/capabilities/ai-toolkit/agents/ai-agent-chatbot) guide, it is recommended to use a frontier model with tool calling capabilities. In particular, the following models have shown positive results:
...
However, in the system prompt, you can reference the [available tools](/content-ai/capabilities/ai-toolkit/agents/tools) by their name. This way, you can instruct the AI model on how and when to use the tools, and guide the AI model to make it more verbose, creative, or thoughtful.
