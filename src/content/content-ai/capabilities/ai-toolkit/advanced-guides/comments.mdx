...
Provide the [tool definitions](/content-ai/capabilities/ai-toolkit/tools/ai-sdk) for the Tiptap AI Toolkit. Enable the `getThreads` and `editThreads` tools so that the AI agent can read and edit comments in your document. Optionally, you can disable the `insertContent` and `applyPatch` tools to prevent the AI from making changes to the document.
