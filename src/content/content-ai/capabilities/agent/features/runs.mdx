...
An AI Agent run is a complete cycle of interaction between the user, the AI Agent, and the document.

It starts when `provider.run()` is called, and ends when the AI Agent has completed the task, needs user review, or encounters an error.

## Lifecycle

1. **Initialization**: The run begins when you call `provider.run()` after adding a user message.
2. **Processing**: The AI Agent calls the LLM model. The LLM responds with a message and/or a tool call.
3. **Tool execution**: The AI Agent calls the tool to read or modify the document.
4. **Loop**: The AI Agent calls the LLM again and executes the next tool, until a termination condition is met.
...
During a run, the AI Agent transitions through different [statuses](/content-ai/capabilities/agent/features/state#status):
...
In its current version, the AI Agent extension does not support running multiple AI Agents within the same editor. If a run is already in progress when `provider.run()` is called, the current run is interrupted before the new run starts.
