...
<Callout title="Content AI components coming soon" variant="hint">
    UI components and templates for Content AI are on the way. If you’re already using our UI Components, you’ll be able to drop them in with minimal changes as soon as they ship.
</Callout>
...
Our new AI-ready UI component library and templates are in progress. Until then, you can explore our [legacy Notion-style template](https://templates.tiptap.dev/9TCLTfrLGt) as a reference for wiring AI into an editor. We no longer maintain this template, so plan to migrate to the upcoming components and templates for an easier upgrade path.
