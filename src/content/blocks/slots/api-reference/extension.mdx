...
`Node` here is the extension type from `@tiptap/core`. See [validators](/blocks/slots/api-reference/validation) and [event payloads](/blocks/slots/events).
...
- `config` (`SlotConfig`): Embedded [configuration](/blocks/slots/concepts). Default: `{ version: 1 }`. Malformed imported JSON is retained for diagnostics.
...
Block Slots require `paragraph` in the schema. See [rendering](/blocks/slots/rendering) for NodeViews and HTML customization.
...
Document edits clear cached validation. Selection changes do not. See [read types](/blocks/slots/api-reference/types).
