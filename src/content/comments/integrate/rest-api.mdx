...
Authenticate with a signed token carrying the `Documents:Api:All` permission, sent as an `Authorization: Bearer <jwt>` header. See [Authentication](/authentication) for how to sign one. This token grants full Document Server API access, so keep it server-side. The previous API secret still works and is documented under [Legacy authentication](/authentication/legacy).
