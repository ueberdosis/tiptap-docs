...
Node Positions ( `NodePos`) describe the specific position of a node, its children, and its parent, providing easy navigatation between them. Node Positions are heavily inspired by the DOM and are based on Prosemirror's [ResolvedPos](https://prosemirror.net/docs/ref/#model.ResolvedPos) implementation.

## Use Node Positions

The easiest way to create a new Node Position is to use the helper functions in the Editor instance. This way you always use the correct editor instance and have direct access to the API.
...
`NodePos` lets you traverse the document in a way similar to the document DOM in your browser. You can access parent nodes, child nodes, and sibling nodes. 
