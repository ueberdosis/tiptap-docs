...
When running Tiptap on the server or in environments without browser APIs, you can provide a custom browser environment. This is essential for server-side content processing.
...
The `browserEnvironment` option accepts a `BrowserEnvironment` object with the following properties:

- `window`: Custom implementation of the [`window`](https://developer.mozilla.org/en-US/docs/Web/API/Window) object
- `domParser`: Custom implementation of the [`DOMParser`](https://developer.mozilla.org/en-US/docs/Web/API/DOMParser) class constructor

If not provided, Tiptap will attempt to use the global browser APIs, which will throw an error in server environments. Learn more about [running Tiptap on the server](/guides/run-tiptap-on-the-server).
