...
They extend all of the options and methods from the [Extension API](/editor/extensions/custom-extensions/create-new/extension) and add a few more specific to marks.
...
Marks inherit the `addDecorations` hook from the [Extension API](/editor/extensions/custom-extensions/create-new/extension#adddecorations). Use it to add **view-only** [decorations](/editor/core-concepts/decorations) — highlights, search results, annotations, and lightweight widgets — that are rendered in the editor but never modify or serialize the document. Read the [Decorations guide](/editor/core-concepts/decorations) and [API reference](/editor/api/decorations) for details.
