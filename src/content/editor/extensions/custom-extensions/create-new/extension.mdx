...
The `addDecorations` method declares **view-only** [decorations](/editor/core-concepts/decorations) — highlights, search results, annotations, and lightweight widgets that are rendered in the editor but never modify or serialize the document. It returns a `DecorationSpec` (or `null` for none), and Tiptap aggregates the declarations from every extension into a single ProseMirror plugin for you.

Build the decorations with the `decoration` factory imported from `@tiptap/core`:
...
The `decoration` factory has three kinds: `decoration.node` (attributes on a node's DOM wrapper), `decoration.inline` (a styled span over a range), and `decoration.widget` (a DOM node or framework component at a position). For large documents you can gate recomputation with `shouldUpdate` or opt into incremental recomputation with `incrementalCreate`, and you can force a rebuild at any time with the `editor.commands.updateDecorations()` command.

<Callout title="Use decorations for view-only markers" variant="info">
  Reach for `addDecorations` when the marker is purely visual. When you need persisted custom document content
  or editable custom blocks, use a [node view](/editor/extensions/custom-extensions/node-views) instead.
</Callout>

Read the [Decorations guide](/editor/core-concepts/decorations) for the three kinds, performance, and framework widgets, and the [Decorations API reference](/editor/api/decorations) for every signature.
...
Learn more about all the options, including the string shorthand syntax, in the [Apply global attributes](/editor/extensions/custom-extensions/extend-existing#apply-global-attributes-to-all-extensions) guide.
