...
The `addDecorations` method adds [decorations](/editor/core-concepts/decorations) to the editor. Use decorations for highlights, search results, and small widgets. They change what users see, but they do not change the saved document.

Build the decorations with the `Decoration` class imported from `@tiptap/core`:
...
The `Decoration` class has three static methods: `Decoration.Node` (attributes on a node's DOM wrapper), `Decoration.Inline` (a styled span over a range), and `Decoration.Widget` (a DOM node or framework component at a position). For large documents you can gate recomputation with `shouldUpdate` or rebuild only edited blocks with `update: 'changedRanges'`. Use `editor.commands.updateDecorations()` to force a full rebuild at any time.

<Callout title="Use decorations for view-only markers" variant="info">
  Reach for `addDecorations` when the marker is purely visual. When you need persisted custom
  document content or editable custom blocks, use a [node
  view](/editor/extensions/custom-extensions/node-views) instead.
</Callout>

Read the [Decorations guide](/editor/core-concepts/decorations) for the three kinds and performance strategies, follow the [Vanilla JS](/guides/decorations-vanilla), [React](/guides/decorations-react), or [Vue 3](/guides/decorations-vue) tutorial, and use the [Decorations API reference](/editor/api/decorations) for every signature.
