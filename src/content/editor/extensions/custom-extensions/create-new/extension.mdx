...
You can also omit the `types` property to apply attributes to all node and mark extensions (excluding the built-in text node by default). Learn more in the [Apply global attributes](/editor/extensions/custom-extensions/extend-existing#apply-global-attributes-to-all-extensions) guide.
