...
Nodes inherit the `addDecorations` hook from the [Extension API](/editor/extensions/custom-extensions/create-new/extension#adddecorations). Use it to add **view-only** [decorations](/editor/core-concepts/decorations) — highlights, search results, annotations, and lightweight widgets — that are rendered in the editor but never modify or serialize the document.

<Callout title="Decorations vs. node views" variant="info">
  Use a [node view](#addnodeview-advanced) when the content is part of the document. Use
  [decorations](/editor/core-concepts/decorations) when the marker is purely visual and lives outside the
  document.
</Callout>
