...
You can omit the `types` property to apply attributes to all node and mark extensions in your editor. By default, this excludes the built-in text node, which ensures that attributes are applied to container elements rather than inline text content.

Here's an example of a `UniversalAttribute` extension that applies an attribute to all extensions:
...
const UniversalAttribute = Extension.create({
  addGlobalAttributes() {
    return [
      {
        // define to specific types or don't pass `types` to apply to all extensions
...
<Callout title="Tip">
This pattern is useful for universal attributes that should be available on all content types throughout your editor. Since the text node is excluded by default, your attributes will be applied to all meaningful container elements (paragraphs, headings, lists, etc.) without affecting plain text nodes.
</Callout>
