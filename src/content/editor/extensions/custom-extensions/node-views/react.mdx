...
For non-leaf node views, `ReactNodeViewRenderer` creates an editable content element inside the wrapper. This element defaults to a `div` for block nodes and a `span` for inline nodes. Use `contentDOMElementTag` to override its tag:

```js
// this will use a header tag for the editable content
return ReactNodeViewRenderer(Component, { contentDOMElementTag: 'header' })
```

## Changing the outer wrapper element

The outer element created by `ReactNodeViewRenderer` defaults to a `div` for block nodes and a `span` for inline nodes. Use the `as` option to override it. This is separate from `contentDOMElementTag`, which controls the inner editable content element:
