...
Sets whether content will be automatically trimmed when it exceeds the limit. When set to `false`, content over the limit is preserved and users can only remove characters until back within the limit.
...
CharacterCount.configure({
  limit: 280,
  autoTrim: false, // Allow content over limit, but restrict to removals only
})
```
