...
Controls whether the initial content is automatically trimmed when it exceeds the configured limit. When set to `false`, initial content over the limit is preserved. While the document remains over the limit, transactions that increase the content size are blocked.
...
CharacterCount.configure({
  limit: 280,
  autoTrim: false, // Preserve initial content over the limit
})
```
