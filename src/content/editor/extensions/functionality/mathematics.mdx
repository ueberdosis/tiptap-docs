...
Tiptap needs to know when the text is mathematical. Therefore, a regular expression pattern allows us to define this shorthand. E.g. using the TeX shorthand `$ … $` (see default below). Matches become decorated – they are not stored as own nodes or marks!

The first content to match the regex's capture group will be rendered by KaTeX.
