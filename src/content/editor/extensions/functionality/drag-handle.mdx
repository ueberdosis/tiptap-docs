...
A performance optimization for the drag image. When set, only the listed CSS properties are copied from `getComputedStyle` when building the drag image clone. By default all ~300+ computed styles are copied per element in the dragged subtree, which can be slow for complex selections. Setting a subset like `['color', 'background-color', 'font-size', 'font-family']` improves drag start performance.

When `undefined` (default), all computed styles are copied as before.
