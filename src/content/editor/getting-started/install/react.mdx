...
<Callout title="Experimental: React-owned rendering" variant="hint">
  `@tiptap/react-experimental` is a drop-in replacement for `@tiptap/react` where React owns the document DOM — no wrapper elements around node views, no portals, and React context flows normally. The public API (`useEditor`, `EditorContent`, node/mark views, menus) is the same. See the [migration guide →](/guides/migrate-to-react-experimental).
</Callout>
...
## Alternative: Experimental React renderer

`@tiptap/react-experimental` is an experimental React binding in which **React owns the editable document DOM**. A `<p>` renders as a `<p>` — no wrapper element and no portal around node views — so React context flows from a parent node view to its children and there is no `flushSync` on the render path. It keeps the same public API as `@tiptap/react` (`useEditor`, `EditorContent`, `<Tiptap>`, node/mark views, menus), so most projects migrate by changing an import. Learn more in the [Migrate to the experimental React renderer guide](/guides/migrate-to-react-experimental).
