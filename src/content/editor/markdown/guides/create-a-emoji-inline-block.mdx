...
export const Emoji = Node.create({
  ...
  // Markdown integration: tokenizer only (parse & render will be added later)
  markdown: {
    name: 'emoji',
...
The `parse` function converts the tokenizer token into a Tiptap node. For an atomic inline node, it should return a node object with the `type` and `attrs`. Here's the full extension now including tokenizer + parse.
...
**Notes:**
- The `parse` function should return an object where `type` matches the node `name`.
...
To support serializing the editor state back to Markdown shortcodes, implement the `render` function. It receives a Tiptap node and should return a Markdown string representing that node. Below is the full extension with tokenizer, parse, and render included.
...
Set the editor content from Markdown that contains emoji shortcodes. Depending on your Markdown integration, pass `asMarkdown: true` or use the API your setup provides:
...
- Unknown names: If a shortcode isn't in `emojiMap`, the node currently renders the raw `name` in the span (you may prefer to show a fallback or remove the node). Validate or normalize names in `tokenize` or `parse` if needed.
