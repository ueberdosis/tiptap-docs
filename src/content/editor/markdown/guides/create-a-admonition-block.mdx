...
Below is a full example that includes the base Node plus the `markdown.tokenizer` added. This gives you full context for how tokenizer integrates with the Node.
...
export const Admonition = Node.create({
  ...
  // Markdown integration: tokenizer only (parse/render not added yet)
  markdown: {
    name: 'admonition',
...
- `tokenize` returns `undefined` when it doesn't match; otherwise it must return a token object with a `raw` string and any fields your `parse` function will expect.
...
The `parse` function receives the token produced by the tokenizer and must return a Tiptap-compatible JSON representation of a node (or nodes). Use the provided `helpers` to parse nested tokens into child content.

Below is the full example containing the base Node, the tokenizer, and now the `parse` function. This shows how the pieces fit together.
...
export const Admonition = Node.create({
  ...
  // Markdown integration: tokenizer + parse
  markdown: {
    name: 'admonition',
...
To serialize content back to Markdown, implement the `render` function. This function receives a Tiptap node and should return the Markdown string representation. Use `helpers.renderChildren` to serialize the node's content.
...
To set editor content from Markdown that uses the admonition syntax, pass the Markdown string and ensure `asMarkdown: true` (depending on your editor integration):
...
- Trailing newlines: Pay attention to trailing newlines consumed by your tokenizer regex. Adjust the regex or the `render` output to match the expectations of your Markdown toolchain.
- Types validation: If you want to enforce only specific types (e.g., `note | warning | tip | danger`) you can validate the `admonitionType` in `tokenize` or in `parse` and fall back to a default when required.
