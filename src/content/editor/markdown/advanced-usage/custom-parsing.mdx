...
```typescript
markdown: {
  parseName: 'heading',  // Token type to handle (optional, default is the extension name)
  renderName: 'customHeading' // Your node type name you want to render in the editor (optional, default is the extension name)
...
When parsing tokens to nodes or marks, it can happen that tokens may not map one-to-one to your node or mark names. In that case, you can use `parseName` and `renderName` to specify which token names to parse and which node/mark names to render.
...
### Example: Parse a `strong` token to a `bold` mark
