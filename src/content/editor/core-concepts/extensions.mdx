...
Extensions can do much more than add new content types. You can:
- **Add attributes** to nodes and marks to store additional data
- **Apply global attributes** to multiple extensions at once (useful for text alignment, line height, and other properties)
- **Add commands** to execute custom editor behaviors
- **Listen to events** like focus, blur, update, and more
- **Add keyboard shortcuts** for quick access to features
- **Add [decorations](/editor/core-concepts/decorations)** to render view-only highlights, annotations, and widgets without changing the document
