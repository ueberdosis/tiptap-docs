...
The [`Table`](/editor/extensions/nodes/table) extension registers a new schema attribute to configure which role a Node has. Allowed values are `table`, `row`, `cell`, and `header_cell`.
