...
meta:
  ...
  description: API reference for the Tiptap Pages extension, covering options, commands, and storage.
  category: Pages
---
...
interface PagesOptions {
  ...
  /** Parse options for header and footer content given as HTML.
   *  Defaults to `{ preserveWhitespace: 'full' }`. */
  headerFooterParseOptions?: ParseOptions

  /** Renders custom controls into the header and footer overlay toolbar, before Close. */
  renderOverlayActions?: RenderOverlayActions

  /** Translatable labels for the header, footer, footnotes and endnotes overlays. */
...
| `headerFooterParseOptions`           | `ParseOptions`                      | `{ preserveWhitespace: 'full' }` | Parse options for header and footer content given as HTML. Repeated spaces are kept by default. |
| `renderOverlayActions`               | `(ctx) => HTMLElement \| { element, destroy? } \| null` | `undefined` | Custom controls inside the header and footer overlay toolbar. See [Customizing the overlay toolbar](/pages/core-concepts/page-header-footer#customizing-the-overlay-toolbar). |
| `overlayLabels`                      | `OverlayLabels`                     | `undefined`          | Translatable labels for every overlay. See [Translating overlay labels](/pages/core-concepts/page-header-footer#translating-overlay-labels). |
...
The full reference for endnote options lives on [Endnotes](/pages/core-concepts/endnotes#complete-options-reference). All options below sit under the `endnotes` key unless noted otherwise.
...
| `unsetHeader`                 | `headerType?: HeaderType`   | Clear a header slot, the default one when omitted             |
| `unsetFooter`                 | `footerType?: FooterType`   | Clear a footer slot, the default one when omitted             |
...
The full reference also lives on [Endnotes](/pages/core-concepts/endnotes#commands-reference).
...
The full reference also lives on [Endnotes](/pages/core-concepts/endnotes#storage-reference).
...
When a header, footer, footnotes, or endnotes overlay is open, these properties reflect which one is active. Useful for building toolbars that follow the focused editor; see [Page header and footer](/pages/core-concepts/page-header-footer#active-editor-state) for a complete toolbar example.
