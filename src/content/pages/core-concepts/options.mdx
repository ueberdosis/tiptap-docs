...
const editor = new Editor({
  extensions: [
    Pages.configure({
      ...
      pageGap: 40,
      header: 'My Project',
      footer: (pageNumber, totalPages) => `Page ${pageNumber} of ${totalPages}`,
...
<Callout title="Note">
  The <code>header</code> and <code>footer</code> options accept either a string (with tokens <code>{page}</code> and <code>{total}</code>) or a function <code>(pageNumber, totalPages) =&gt; string</code> for dynamic content.
</Callout> 