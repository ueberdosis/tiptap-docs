...
Use PageBreak as a standalone extension for a visual page break indicator. Pair it with [ConvertKit](/conversion/import/docx/convertkit) for the editor schema:
...
When both PageBreak and Pages are registered, PageBreak automatically operates in Pages mode. Use the standard [Pages stack](/pages/getting-started/install) — ConvertKit + TableKit + Pages — and add PageBreak alongside:
...
The Tiptap [conversion service](/conversion/import/docx/editor-extension) emits `pageBreak` nodes when importing `.docx` files that contain page breaks. These map directly to this extension's node type, so imported documents preserve their page break positions.
