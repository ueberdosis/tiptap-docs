...
meta:
  ...
  description: Add Word-like footnotes to Tiptap Pages with per-page footnote areas, automatic numbering, in-place editing, and DOCX round-trip.
  category: Pages
sidebars:
  hideSecondary: true
---
...
<Callout title="Looking for endnotes?" variant="hint">
  Notes that gather into a single list at the end of the document are
  [endnotes](/pages/core-concepts/endnotes). Footnotes and endnotes are independent and can be used
...
- **Footnotes consume page space.** The area grows with its content and the body content area shrinks accordingly, so adding footnotes to a full page pushes body text onto the next page.
...
- **Numbering is automatic and continuous.** Footnotes are numbered `1, 2, 3…` in document order. Inserting a footnote between two existing ones renumbers everything after it; deleting one closes the gap. Numbers are computed, never stored, so they're always correct.
...
Close the editor with <kbd>Escape</kbd>, the close button, or by double-clicking outside it. While a footnotes editor is open, the main document editor is temporarily non-editable, the same behavior as the header and footer editors.
...
The footnotes overlay label and its Close button are translatable through `overlayLabels`. See [Translating overlay labels](/pages/core-concepts/page-header-footer#translating-overlay-labels).
...
Like headers and footers, you can keep the footnotes editor open when the user double-clicks outside it. This is useful when your own toolbar sits outside the editor:
...
Clicking a footnote marker in the body scrolls the page so its footnote is visible, which is handy in long documents.
...
- The page body shrinks as footnotes grow, so pagination accounts for footnote space.
