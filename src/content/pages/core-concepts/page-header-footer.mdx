...
<Callout title="DOCX import" variant="info">
  When importing a Word file, `PAGE` and `NUMPAGES` fields come back as the canonical text tokens `{page}` and `{numpages}`. The default Pages config recognizes `{page}` but **not** `{numpages}`. To make Word's `NUMPAGES` field render live in the editor preview after an import, configure `placeholders: { total: 'numpages' }`. See [DOCX import → page-number fields](/conversion/import/docx/editor-extension#page-number-fields) for the full picture.
</Callout>
...
Header and footer editors participate in collaboration alongside the main document. Set up a collaborative provider as you would for any Pages editor — no extra wiring is required to bring headers and footers into the same session. See [Adding collaboration to Pages](/pages/guides/collaboration-with-pages) for the full setup walkthrough.
