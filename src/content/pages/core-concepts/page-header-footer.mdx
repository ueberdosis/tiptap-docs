...
Header and footer editors participate in collaboration alongside the main document. Every subtype (default, first page, odd, even) collaborates independently and the `differentFirstPage` / `differentOddEven` toggles sync across clients. To opt in, pass `headerFooterExtensions` as a **callback** that attaches a `Collaboration` extension to each sub-editor; the static-array form does not enable collaboration on the sub-editors. See [Adding collaboration to Pages](/pages/guides/collaboration-with-pages) for the full setup walkthrough.
