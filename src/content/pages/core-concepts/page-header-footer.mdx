...
There are two ways to add controls. Pick by where the control belongs.
...
```ts
Pages.configure({
  renderOverlayActions: ({ innerEditor }) => {
...
Keep state outside the returned element. Every rebuild destroys the slot and renders it again.

### Translating overlay labels

Every overlay label goes through `overlayLabels`. Each label entry is a function, so any i18n library fits. Missing entries keep the English defaults. `close` is a string and becomes the accessible name and tooltip of every Close button.
