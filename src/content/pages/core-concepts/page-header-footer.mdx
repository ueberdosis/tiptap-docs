...
HTML strings are parsed with the browser's whitespace rules, so repeated spaces collapse to one. To keep them, pass parse options for the header and footer editors:

```js
Pages.configure({
  header: '<p>Ref.   2026-09</p>',
  headerFooterParseOptions: { preserveWhitespace: 'full' },
})
```

This applies to HTML given to the `header` and `footer` options, to `setHeader` and `setFooter`, and to the inline editors. Content given as JSON is never re-parsed, so it keeps its spacing without the option. Opening and closing the inline editor does not change stored content either way.
