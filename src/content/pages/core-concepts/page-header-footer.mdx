...
### Whitespace in HTML strings

Repeated spaces in header and footer HTML are kept. `<p>Ref.   2026-09</p>` stays three spaces wide, in the `header` and `footer` options, in `setHeader` and `setFooter`, and in the inline editors. Content given as JSON is never re-parsed, so it keeps its spacing too. Opening and closing the inline editor does not change stored content.

To get the browser's default whitespace rules back, where repeated spaces collapse to one, pass your own parse options:

```js
Pages.configure({
  header: '<p>Ref.   2026-09</p>',
  headerFooterParseOptions: { preserveWhitespace: false },
})
```
...
## Clearing a header or footer

`unsetHeader` and `unsetFooter` empty a slot and give its reserved page space back. Without an argument they clear the default slot. Pass `'first'`, `'odd'` or `'even'` for a variant.

```js
editor.commands.unsetHeader()
editor.commands.unsetFooter('first')
```

In collaboration the cleared slot reaches every connected client, like `setHeader` does. DOCX import uses these commands for every slot the imported file does not carry.
...
### Customizing the overlay toolbar

Two ways to add controls, pick by where the control belongs.

- Use the pattern above, an external toolbar that follows `editor.storage.pages.activeEditor`, for formatting controls that also apply to the document body.
- Use `renderOverlayActions` for controls that belong inside the header or footer overlay itself, next to the Close button.

`renderOverlayActions` runs every time the overlay toolbar is built: on open, on zoom change, and when the header type changes. Return an element, or `{ element, destroy }` when you need a teardown hook, for example to unmount a React portal. Return `null` to render nothing.

```ts
Pages.configure({
  renderOverlayActions: ({ innerEditor, kind, subType, pageNumber, close }) => {
    const button = document.createElement('button')
    button.type = 'button'
    button.textContent = 'Insert date'
    button.onclick = () => {
      innerEditor.chain().focus().insertContent(new Date().toLocaleDateString()).run()
    }
    return button
  },
})
...
The context gives you the parent `editor`, the `innerEditor` being edited, the `kind` (`'header'` or `'footer'`), the `subType` (`'default'`, `'first'`, `'odd'` or `'even'`), the `pageNumber`, and `close()`.

Keep state outside the returned element. The slot is destroyed and rendered again on every rebuild.

### Translating overlay labels

Every overlay label goes through `overlayLabels`. Each entry is a function, so any i18n library fits. Missing entries keep the English defaults. `close` is a string and becomes the accessible name and tooltip of every Close button.

```ts
Pages.configure({
  overlayLabels: {
    header: ({ subType, pageNumber }) =>
      subType === 'first' ? t('pages.header.first') : t('pages.header.page', { pageNumber }),
    footer: ({ subType, pageNumber }) => t('pages.footer', { subType, pageNumber }),
    footnotes: ({ pageNumber }) => t('pages.footnotes', { pageNumber }),
    endnotes: () => t('pages.endnotes'),
...
| `unsetHeader`              | `headerType?: HeaderType`   | Clear a header slot. The default slot when omitted, or `'first'`, `'odd'`, `'even'`. Reserved header space goes back to zero. |
| `unsetFooter`              | `footerType?: FooterType`   | Clear a footer slot. The default slot when omitted, or `'first'`, `'odd'`, `'even'`. Reserved footer space goes back to zero. |
