...
When the document is exported to `.docx`, each `{page}` / `{total}` is converted into a live Word `PAGE` / `NUMPAGES` field, so Word recalculates the numbers on print or repagination.
...
<Callout title="DOCX import" variant="info">
  When importing a Word file, the import service emits the canonical token names `{page}` and `{numpages}` (verbatim from Word's field names). The default Pages config recognizes `{page}` but **not** `{numpages}`. To make Word's `NUMPAGES` field render live in the editor preview after an import, configure `placeholders: { total: 'numpages' }`. See [DOCX import → page-number fields](/conversion/import/docx/editor-extension#page-number-fields) for the full picture.
</Callout>
