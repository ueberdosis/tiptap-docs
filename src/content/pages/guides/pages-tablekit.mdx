...
By default, a table row that does not fit on the current page moves to the next page as a whole. This differs from Microsoft Word, which splits a tall row across pages.
...
There is now an experimental opt-in that removes this limit for rows whose cells hold ordinary flowing content. See [Rows that span a page break](/pages/guides/row-fragmentation).
...
Each of those keys forwards to the underlying extension's standard options.

PagesTableKit adds one option of its own:

```ts
TableKit.configure({
  // Let a tall row continue on the next page instead of moving down whole.
  // Experimental, off by default.
  experimentalRowFragmentation: true,
})
```

See [Rows that span a page break](/pages/guides/row-fragmentation) for what it does and which rows it applies to.

## Related

- [Pages overview](/pages/getting-started/overview): the pagination extension PagesTableKit requires.
- [Rows that span a page break](/pages/guides/row-fragmentation): let a tall row continue on the next page.
