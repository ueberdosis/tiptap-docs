...
- **Endnotes**: Word-like endnotes collected at the end of the document with lowercase Roman numerals, automatic numbering, and in-place editing. See [Endnotes](/pages/core-concepts/endnotes).
- **Edit-in-place**: Double-click a header, footer, footnotes, or endnotes area to open a fully featured Tiptap editor in the overlay.
...
- Word-like [endnotes](/pages/core-concepts/endnotes): a single list at the end of the document with lowercase Roman numerals and automatic numbering, flowing across pages with full page chrome.
- Live editing of headers, footers, footnotes, and endnotes in dedicated overlays. They participate in collaboration alongside the main document. See [Adding collaboration to Pages](/pages/guides/collaboration-with-pages).
- Programmatic control: open or close a header/footer/footnotes/endnotes editor, react to page format changes, react to zoom changes.
- Lock or unlock editing per overlay, at config-time or runtime via the `editableHeader` / `editableFooter` / `footnotes.editable` / `endnotes.editable` options and the `setHeaderEditable` / `setFooterEditable` / `setFootnotesEditable` / `setEndnotesEditable` commands.
...
- Round-trip with Tiptap Conversion: imported DOCX headers, footers, footnotes, and endnotes are auto-applied, and exported DOCX, PDF, ODT, and EPUB files carry headers, footers, and pagination through. DOCX export writes real Word footnotes and endnotes.
...
- **Non-splittable blocks larger than a page cause an infinite layout loop.** This is the single most important limitation to know about before adopting Pages. Figures, positioned containers, nested tables and most other Block Formatting Context (BFC) elements cannot be split across pages. The layout can only move the whole block to the next page. If the block is taller than the page itself, it can't fit there either, and the layout keeps trying to move it forward and never stabilises. The result is an unresponsive editor. This is a hard limit of the current approach to splitting content between pages, not a near-term roadmap item. See [Limitations](/pages/core-concepts/limitations#oversized-non-splittable-blocks-cause-an-infinite-layout-loop) for the full treatment, including how to detect and prevent it. Tall **table rows** are the exception: PagesTableKit can split one across pages when its cells hold ordinary flowing content, see [Rows that span a page break](/pages/guides/pages-tablekit#rows-that-span-a-page-break).
