...
Pages on its own makes no network calls and needs no credentials. The moment you add Tiptap Conversion (DOCX import, PDF export, etc.) on top of Pages, you'll need a signed token. See the [Conversion install guide](/conversion/getting-started/install) for setup. The configuration there carries over without modification.
