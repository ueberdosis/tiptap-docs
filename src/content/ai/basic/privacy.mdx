---
title: Basic AI Generation privacy
meta:
  title: Privacy | Tiptap Basic AI Generation
  description: Learn how Tiptap Basic AI Generation protects your data with encrypted storage, secure proxying, and optional on-premise deployment for full control.
  category: Basic AI Generation
---

At Tiptap we value your privacy. So when using Tiptap Basic AI Generation that topic may raise some questions which we like to address in detail below.

## Cloud integration

The Tiptap AI Server **acts as a proxy** between your client (the Tiptap Editor) and OpenAI. This is done to protect your OpenAI secret from the client, as it is an anti-pattern to share secrets publicly and user-facing.
...
3. The backend verifies the signed token and its permissions, then continues the request or aborts with an error code
...
- Your OpenAI API Key
- Your JWT secret (for integrations still using the previous authentication)
