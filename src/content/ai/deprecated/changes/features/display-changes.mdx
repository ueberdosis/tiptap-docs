...
For more advanced styles, use the `getCustomDecorations` [configuration option](/ai/deprecated/changes/configure#custom-styles).
...
To learn how to show a popover when you select a change, follow [this guide](/ai/deprecated/changes/features/display-changes#show-a-popover-when-a-change-is-selected).
...
We recommend using the [Floating UI](https://floating-ui.com/) library to display the popover. You can see an example on how to do it [in the demo](/ai/deprecated/changes/overview).

## Display changes in a sidebar outside the editor

You can access the changes data from the extension's [storage object](/ai/deprecated/changes/api-reference#extension-storage):
