...
The most common pattern is to call `startTrackingAiChanges` immediately before generating AI content. This allows you to capture all AI-generated changes for review. For example, you can chain it with the [AI Generation extension commands](/ai/simple-generation/text-generation/built-in-commands):
...
- `getIsShowingAiChanges`: Returns a boolean that indicates if the editor is currently showing the AI changes. AI changes [can be temporarily hidden](/ai/deprecated/changes/features/display-changes#hide-and-show-changes) with the `setShowAiChanges` command.
...
To learn how to use these methods to display changes in your editor, see the [Display AI Changes](/ai/deprecated/changes/features/display-changes) guide.
...
You can learn more about the AI Changes extension commands in the [API Reference](/ai/deprecated/changes/api-reference).
