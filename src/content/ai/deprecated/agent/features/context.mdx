...
3. In [your backend](/ai/deprecated/agent/custom-llms/overview), define a [custom server-side tool](/ai/deprecated/agent/custom-llms/server-side-tools) that lets your AI agent read the contents of the file.

The same applies to URLs. If the user provides a URL, you can include it as a string in the `context` option and define a [custom tool](/ai/deprecated/agent/custom-llms/server-side-tools) to search the web and retrieve the content of that URL.

## Add the editor's selection to context

To make the AI agent aware of the selected content in the editor, activate [selection awareness](/ai/deprecated/agent/features/selection-awareness).
