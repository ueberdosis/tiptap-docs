---
title: Install the AI Agent extension
meta:
  title: Install the AI Agent extension | Tiptap Content AI
  description: Learn how to install and set up the AI Agent extension for Tiptap.
  category: Content AI
tags:
  - type: beta
  - type: deprecated
---

import { Callout } from '@/components/ui/Callout'
import { EnterpriseCalloutAuto } from '@/components/EnterpriseCalloutAuto'

<Callout title="Deprecated" variant="warning">
  This extension is being phased out in favor of the AI Toolkit, which offers enhanced capabilities
  and a modernized API. While you can continue using this extension, we recommend exploring the AI
  Toolkit for new implementations.
</Callout>

<EnterpriseCalloutAuto variant="deprecated" renderMode="sidebar" />

## Install the package

The AI Agent extension is published in Tiptap's private npm registry. Integrate the extension by following the [private registry guide](/guides/pro-extensions).

## Set up the provider

The provider object manages the state of the AI Agent extension. The easiest way to get started with the AI Agent extension is to use the Tiptap Cloud provider:

```tsx
import { AiAgentProvider } from '@tiptap-pro/extension-ai-agent'

const provider = new AiAgentProvider({
  // Configure Tiptap Cloud (see next section)
  token: 'TOKEN_HERE',
})
```

The AI Agent extension works out-of-the-box [with Tiptap Cloud](/ai/deprecated/agent/use-with-content-ai-cloud) as a backend.

To add text-editing capabilities to your custom AI agent that runs in your backend, read [the integration guide](/ai/deprecated/agent/custom-llms/overview).

## Import the extension in your editor

Add `AiAgent` to the list of extensions and configure it with the provider.

```tsx
import { Editor } from '@tiptap/core'
import StarterKit from '@tiptap/starter-kit'
import AiAgent from '@tiptap-pro/extension-ai-agent'

const editor = new Editor({
  extensions: [
    StarterKit,
    AiAgent.configure({
      provider,
      // … other options
    }),
    // … more extensions
  ],
})
```

## Call the AI Agent extension

Call the methods of the provider to interact with the AI Agent extension.

```tsx
provider.addUserMessage(
  'Correct the spelling and grammar mistakes. Then, find the key phrases in the text and format them in bold.',
)
provider.run()
```

## Display the chat conversation

You can access the AI Agent extension state (including the chat conversations) and display it in the UI:

```tsx
provider.state.messages
```

You can also subscribe to events and update the UI when they are triggered:

```tsx
// Subscribe to state changes
provider.on('stateChange', (newState, previousState, context) => {
  console.log('State changed:', newState)
  // Update your UI based on the new state
})

// Subscribe to loading errors
provider.on('loadingError', (error, context) => {
  console.error('Error occurred:', error)
  // Show error message to the user
})
```
