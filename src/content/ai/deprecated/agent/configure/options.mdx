...
meta:
  title: Configure AI Agent extension | Tiptap Content AI
...
You can [define a custom system prompt](/ai/deprecated/agent/configure/system-prompt) for the AI agent when using it with Tiptap Cloud.
...
You can provide initial chat messages to populate the conversation when the extension is first loaded.
...
The `reviewOptions` option lets you define a workflow for reviewing, accepting and rejecting AI-generated changes. Learn more in the [Review AI-generated changes](/ai/deprecated/agent/review) guide.
...
The `AiAgentProvider` supports several event handlers to help you respond to different states and actions:

### State change handler

The `onStateChange` handler is called whenever the state of the AI Agent extension changes.
...
The `onLoadingError` handler is called when there's an error loading the AI Agent extension.
...
The `onStopRunning` handler is called when the AI Agent extension stops running and its [lifecycle](/ai/deprecated/agent/features/lifecycle) is finished.

```ts
const provider = new AiAgentProvider({
  onStopRunning: (context) => {
    console.log('The AI Agent extension finished running')
    playCompletionSound()
  },
  // ... Other options
})
...
You can specify a custom API endpoint for the AI Agent extension by setting the `baseUrl` option. This is useful if you're running your own instance of the Tiptap Content AI service.
...
These configuration options let you integrate the AI Agent extension with your custom AI agent instead of Tiptap Cloud's managed AI agent. To learn more, read the [Custom AI agent integration guide](/ai/deprecated/agent/custom-llms/overview).

### Custom resolver

The `resolver` option allows you to integrate the AI Agent extension with your own backend service. This function is responsible for sending the chat messages to the LLM and returning the response.
...
By default, the AI Agent extension uses the `defaultAiAgentResolver` which sends the chat messages to Tiptap Cloud.

### Custom tools

The `toolHandlers` option allows you to define custom client-side tools that the AI agent can call. This option only works when you're using the AI Agent extension with your own backend. To learn how to set up custom tools, read the [Custom AI agent integration guide](/ai/deprecated/agent/custom-llms).
...
Configure the AI Agent extension so that it can [generate and understand custom nodes](/ai/deprecated/agent/configure/custom-nodes).
...
| `resolver`                      | `Function`                 | `defaultAiAgentResolver`   | Function to resolve requests with custom backend                                  |
| `toolHandlers`                  | `AiAgentToolCallHandler[]` | `toolHandlersStarterKit()` | Handlers for custom tools                                                         |
| `onStateChange`                 | `Function`                 | `undefined`                | Called when the state of the AI Agent extension changes                           |
| `onLoadingError`                | `Function`                 | `undefined`                | Called when there's an error loading the AI Agent extension                       |
...
| `onStopRunning`                 | `Function`                 | `undefined`                | Called when the AI Agent extension stops running                                  |
| `systemPrompt`                  | `string`                   | `undefined`                | Custom system prompt for the AI Agent extension when using it with Tiptap Cloud   |
