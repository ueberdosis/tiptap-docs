...
meta:
  title: Read the document | Tiptap AIUmbrellaTerm
...
- `reviewOptions?` ([`ReviewOptions`](/content-ai/capabilities/ai-toolkit/api-reference/review-options)): Review mode options used to decide which document representation should be read.
...
- `options?` (`TiptapReadChunksOptions`): Options for the `tiptapReadChunks` method
  - `chunkSize?` (`number`): Max characters per chunk. Defaults to 32,000 characters (8000 tokens \* 4 characters per token)
  - `reviewOptions?` ([`ReviewOptions`](/content-ai/capabilities/ai-toolkit/api-reference/review-options)): Review mode options used to decide which document representation should be chunked.
