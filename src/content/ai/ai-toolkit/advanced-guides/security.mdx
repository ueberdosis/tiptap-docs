...
Additionally, require explicit approval from the user before the AI takes any destructive or consequential action. One way to do so is to [integrate the AI Toolkit with Tracked Changes](/ai/ai-toolkit/agents/tracked-changes), so that the user can accept and reject every document edit that the AI makes.
