...
meta:
  title: Selection awareness | Tiptap AI Toolkit
...
<Callout title="Continuation from the AI agent chatbot guide" variant="info">
  This guide continues the [AI agent chatbot guide](/ai/ai-toolkit/client/agents/ai-agent-chatbot).
  Read it first.
</Callout>
...
The tool can then be executed on the client using the `executeTool` method. It does not require any parameters.
...
- Learn more about the `getActiveSelection` and `setActiveSelection` methods in the [API reference](/ai/ai-toolkit/client/api-reference/execute-tool#setactiveselection).
