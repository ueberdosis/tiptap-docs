...
meta:
  title: AI agent tools — Other providers | Tiptap AI Toolkit
...
Use the `toolDefinitions()` function from the `@tiptap-pro/client-ai-toolkit-tool-definitions` package to get the tool definitions in a generic format:

```ts
import { toolDefinitions } from '@tiptap-pro/client-ai-toolkit-tool-definitions'
...
If your backend is in another programming language than TypeScript, you can use the CLI to generate a JSON file with the tool definitions. See [this guide](/ai/ai-toolkit/client/advanced-guides/non-typescript-backends).
...
An array containing the enabled tool definitions in a generic format. Each tool definition is an object with `name`, `description`, and `inputSchema` properties. To see the full list of tools, see the [Available tools](/ai/ai-toolkit/client/agents/tools) page.
