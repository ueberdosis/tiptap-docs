...
title: Suggestions
meta:
  title: Review AI changes with suggestions | Tiptap AI Toolkit
...
<Callout title="Continuation from the AI agent chatbot guide" variant="info">
  ...
  guide](/ai/ai-toolkit/client/agents/ai-agent-chatbot). Read it first.
</Callout>
...
<CodeDemo isLarge path="" isScrollable src="https://ai-toolkit-demos.vercel.app/preview-changes" />
...
After executing the tool, several **suggestions** will be displayed inside the editor. Each suggestion represents a change that the AI model made to the document.

You can keep the conversation going while users review suggestions. Call `addToolOutput` immediately after each tool call, and show the review UI in parallel.

Edit the code so that the chatbot continues running while showing a review UI:
...
import { AiToolkit, getAiToolkit, type SuggestionFeedbackEvent } from '@tiptap-pro/client-ai-toolkit'
...
See a complete reference of the `reviewOptions` parameter in the [API reference](/ai/ai-toolkit/client/api-reference/execute-tool) for the `executeTool` method.

You can combine this guide with [Streaming](/ai/ai-toolkit/client/agents/streaming) to stream operations and keep the assistant responsive while suggestions are reviewed.

### Customize how suggestions are displayed

When reviewing changes, the AI Toolkit displays the preview of the changes as [suggestions](/ai/ai-toolkit/client/api-reference/suggestions).

You can customize how these suggestions are displayed, by setting the `reviewOptions.displayOptions` parameter. You can even [render React components](/ai/ai-toolkit/client/agents/review-changes/style-suggestions) inside them.
