...
meta:
  title: Multi-document AI agent | Tiptap AI Toolkit
...
Include the [tool definitions](/ai/ai-toolkit/client/agents/tools/ai-sdk) for the Tiptap AI Toolkit (`...toolDefinitions()`), and combine them with the custom tools we'll define below:
...
export default function Page() {
  ...
  const handleToolCall = (toolCall: { toolName: string; input: unknown }): string => {
    ...
    if (toolName === 'createDocument') {
      ...
      return createDocument(createDocumentInput.documentName, createDocumentInput.content)
