...
```bash
npm install @tiptap-pro/client-ai-toolkit @tiptap-pro/client-ai-toolkit-ai-sdk
```

## API endpoint

Create an API endpoint that uses the [Vercel AI SDK](https://ai-sdk.dev/) to call the OpenAI model. Include the [tool definitions](/ai/ai-toolkit/client/agents/tools/ai-sdk) for the Tiptap AI Toolkit.
...
When the AI model receives a request, it will decide to call the available tools to read the document and edit it. See the [list of available tools](/ai/ai-toolkit/client/agents/tools) for more details.

## Client-side setup

When the AI model decides to call a tool, it needs to be executed on the client. This is done with the [`executeTool` method](/ai/ai-toolkit/client/api-reference/execute-tool):
...
- Let your users review AI-generated changes with the [review changes guide](/ai/ai-toolkit/client/agents/review-changes)
- Add real-time tool streaming to see changes as they happen with the [streaming guide](/ai/ai-toolkit/client/agents/streaming)
- Does your document contain custom nodes and marks? Learn how make your AI agent understand them with the [schema awareness guide](/ai/ai-toolkit/client/api-reference/schema-awareness).
