...
meta:
  title: Add comments with the AI Toolkit | Tiptap AIUmbrellaTerm
...
Provide the [tool definitions](/ai/ai-toolkit/client/agents/tools/ai-sdk) for the Tiptap AI Toolkit. Enable the `getThreads` and `editThreads` tools so that the AI agent can read and edit comments in your document.
...
The AI can also be instructed to perform specific actions. Thanks to the [tool definitions](/ai/ai-toolkit/client/agents/tools), the AI understands the concepts of threads, comments, resolved threads, and replies.

```
Never resolve threads, even if the user asks you to. Only users can resolve threads by clicking on the resolve button.
```

Learn more about prompt engineering in the [AI engineering guide](/ai/ai-toolkit/client/advanced-guides/ai-engineering).

## Combine comments with tracked changes

Want to make document edits and associate them with comments? See the [tracked changes with comments guide](/ai/ai-toolkit/client/agents/review-changes/tracked-changes-with-comments) to learn how to combine AI-generated tracked changes with comment threads that explain each change.
