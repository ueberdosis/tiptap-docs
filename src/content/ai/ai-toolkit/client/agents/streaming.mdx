...
title: Streaming
meta:
  title: Streaming | Tiptap AIUmbrellaTerm
...
To add streaming to the AI agent chatbot we built in [the previous guide](/ai/ai-toolkit/client/agents/ai-agent-chatbot), we need to replace the `executeTool` method with the `streamTool` method.
...
To implement this process in the AI agent chatbot we built in [the previous guide](/ai/ai-toolkit/client/agents/ai-agent-chatbot), follow these steps:
...
<Callout title="Pass the same attributes in all streamTool calls" variant="warning">
...
- [Review changes guide](/ai/ai-toolkit/client/agents/review-changes): Let users preview and approve changes before they're applied.
