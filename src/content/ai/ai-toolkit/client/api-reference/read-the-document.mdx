...
Used in the [Proofreader workflow](/ai/ai-toolkit/client/workflows/proofreader) and other workflows.
...
- `reviewOptions?` ([`ReviewOptions`](/ai/ai-toolkit/client/api-reference/review-options)): Review mode options used to decide which document representation should be read.
...
- `options?` (`TiptapReadChunksOptions`): Options for the `tiptapReadChunks` method
  - `chunkSize?` (`number`): Max characters per chunk. Defaults to 32,000 characters (8000 tokens \* 4 characters per token)
  - `reviewOptions?` ([`ReviewOptions`](/ai/ai-toolkit/client/api-reference/review-options)): Review mode options used to decide which document representation should be chunked.
