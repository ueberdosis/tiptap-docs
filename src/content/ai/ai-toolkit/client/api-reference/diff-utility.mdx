...
- `nodeComparators?` (`Record<string, (nodeA: Node, nodeB: Node) => boolean | undefined>`): *Experimental. We recommend using this feature only when strictly necessary. Prefer modeling your document so that equal content has the same markup.* Attribute comparators keyed by node type, supported in all diff modes, including nested nodes. Callbacks receive ProseMirror nodes after `ignoreAttributes` and `ignoreMarks` are applied, and run only when types and marks match but attributes differ. Return `true` to allow the attribute difference; `false` or `undefined` keeps it. Types, marks, text, and children still compare normally. Callbacks must be synchronous and deterministic. This changes comparison only, not metadata preservation when applying replacements.
