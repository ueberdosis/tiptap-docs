...
Comparators apply to inline and block nodes in `inline` and `smart` modes, including nested blocks. In smart mode, equivalent block attributes allow the blocks to match, while changes to their children are still diffed normally. Comparators have no effect in `block` mode.
