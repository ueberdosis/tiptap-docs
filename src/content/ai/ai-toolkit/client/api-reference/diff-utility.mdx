...
const compareMath: NodeComparator = ({ left, right }) =>
  left.attrs.expression === right.attrs.expression
...
Each callback receives `{ left, right }`, where both values are ProseMirror nodes of the same type with matching marks. The nodes have already been normalized using `ignoreAttributes` and `ignoreMarks`. If your comparator needs an attribute such as `id`, remove that attribute from `ignoreAttributes`.
