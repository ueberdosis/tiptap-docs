...
Comparators apply to inline and block nodes in all three modes, including nested blocks. In `smart` mode, equivalent block attributes allow the blocks to match, while changes to their children are still diffed normally. In `block` mode, equivalent blocks produce no diff, but genuine differences still replace the entire top-level block.
