...
meta:
  title: Comments workflow | Tiptap AI Toolkit
...
```bash
npm install @tiptap-pro/client-ai-toolkit @tiptap-pro/client-ai-toolkit-tool-definitions @tiptap-pro/extension-comments
```
...
import { AiToolkit, getAiToolkit, editThreadsWorkflowOutputSchema } from '@tiptap-pro/client-ai-toolkit'
...
To add comments to only a sub-section of the document, set the `range` option on `tiptapRead`. This argument takes a [Range](/ai/ai-toolkit/client/advanced-guides/concepts#range) with the region to read.
...
- [API reference](/ai/ai-toolkit/client/api-reference/workflows#getthreads) of the `getThreads` method
- [API reference](/ai/ai-toolkit/client/api-reference/workflows#editthreadsworkflow) of the `editThreadsWorkflow` method
