---
title: REST API (legacy v3 endpoints)
meta:
  title: REST API (legacy) | Tiptap Server AI Toolkit
...
<Callout title="Deprecated endpoints" variant="warning">
  These API endpoints are deprecated, see the [new API
  endpoints](/ai/ai-toolkit/api-reference/rest-api).
</Callout>
...
See the [authorization guide](/ai/ai-toolkit/install#set-up-authorization) for more information.

These endpoints also accept the new [authentication](/authentication). The exception is `stream-tool`, which requires the new authentication and does not accept legacy tokens.
...
- `"shorthandV2"`: Tiptap Shorthand V2, a token-efficient format for prompts and model output
...
Returns the available [tool definitions](#available-tools)
for AI agents.

Request body:

- `editorContext` (`EditorContext`, required). Editor context obtained from the `getEditorContext` function. See [installation guide](/ai/ai-toolkit/install).
...
- `format?` (`'json' | 'shorthand' | 'shorthandV2'`, optional, default: `'json'`)
...
`editThreads`. See the
[tools reference](#available-tools) for every
...
- `input` (required): Tool input generated by the AI model. The shape depends on
  `toolName`; see the [tools reference](#available-tools).
...
- `toolConfig?` (optional): Tool-specific parameters provided by the developer, not by
  the AI model. See the [tools reference](#available-tools).
- `format?` (`'json' | 'shorthand' | 'shorthandV2'`, optional, default: `'json'`)
- `reviewOptions?` ([`ReviewOptions`](/ai/ai-toolkit/api-reference/review-options), optional, default: `{ mode: 'disabled' }`)
...
- `before` (`object`): the document as it was before the tool ran; always returned
- `document` (`object | null`): the document after the tool ran, or `null` when the document did not change
...
- `format?` (`'json' | 'shorthand' | 'shorthandV2'`, optional, default: `'json'`). The `proofreader` workflow only accepts `'shorthand'` or `'shorthandV2'`.
...
- `schemaAwarenessData` (`SchemaAwarenessData`, required)
- `format` (`'json' | 'shorthand' | 'shorthandV2'`, required)
...
- `schemaAwarenessData` (`SchemaAwarenessData`, required)
- `format` (`'shorthand' | 'shorthandV2'`, required)
- `chunkSize?` (`number`, optional, default: `32000`)
- `reviewOptions?` ([`ReviewOptions`](/ai/ai-toolkit/api-reference/review-options), optional, default: `{ mode: 'disabled' }`)
- exactly one of:
  - `document`
...
- `threads` (`array`): Returned thread objects.
  ...
  - `content` (`JSONContent[] | string | null`): Content marked by the thread. When `format` is
    `json`, this is a Tiptap JSON fragment. When `format` is `shorthand` or `shorthandV2`, this is a Tiptap
