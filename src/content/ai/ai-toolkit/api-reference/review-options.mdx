...
- `trackedChangesOptions?` (`{ userId?: string, userMetadata?: Record<string, unknown> | null }`): Author metadata for the `trackedChanges` mode. `userId` identifies the suggestion author. Defaults to the request `user` when ommitted and, if that is not available, it defaults to `'AI'`. `userMetadata` stores additional author information such as a display name. Defaults to `null`.
