...
Reads a collaborator's live editor selection from a cloud document, marking the selected span so the
AI can edit only what the user selected.

<Callout title="Requires a live document session" variant="warning">
  `readSelection` reads the selection from live collaboration awareness. It requires a Tiptap Cloud
  document, the default document field, and server document sessions enabled.
</Callout>

Tool config (`tool.config`):

- `user` (`string`, required): The `id` of the collaborator whose selection to read, matched against
  the `user.id` published in collaboration awareness by the Collaboration Caret.

Tool output (`tool.output`):

Success response (selection found):

- `success` (`true`): Indicates that the request succeeded.
- `isEmpty` (`false`): A non-empty selection was resolved.
- `content` (`JSONContent[] | string`): The complete top-level nodes containing the selection, with
  `selectionStart` and `selectionEnd` marker nodes at the exact boundaries. When `format` is `json`,
  this is a Tiptap JSON fragment. When `format` is `shorthand`, this is a Tiptap Shorthand string
  with `$selectionStart[]` and `$selectionEnd[]` markers.
- `nodeRange` (`[number, number]`): Half-open top-level node range containing the selection, in the
  same form as `tiptapRead`.

Success response (no selection):

- `success` (`true`): Indicates that the request succeeded.
- `isEmpty` (`true`): The requested user has no usable selection.
