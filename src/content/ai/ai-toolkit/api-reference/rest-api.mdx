...
- `user` (`string`, optional): Author of comments and threads the AI creates (for example with
  `editThreads`). It does not attribute `tiptapEdit` content changes; for tracked-change authorship
...
- `before` (`object`): The document as it was before the tool ran. Always returned, so you can
  inspect the document state even when nothing changed.
- `document` (`object | null`): The document after the tool ran, or `null` when the document did not
  change.

Both are full Tiptap JSON documents, so you can diff `before` against `document` to inspect exactly
what the tool changed.

<Callout title="Node hashes in the diff" variant="hint">
  The first edit of a document that was never read may add `_hash` attributes to nodes. In that case
  `document` includes the hashes and `before` does not, so a diff shows them as changes. Read the
  document with `tiptapRead` first (which persists the hashes) to avoid this.
</Callout>
