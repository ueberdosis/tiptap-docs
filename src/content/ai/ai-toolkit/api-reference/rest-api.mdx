...
- `before` (`object | null`): The document before the tool ran, or `null` when the document did not
  change.
- `document` (`object | null`): The document after the tool ran, or `null` when the document did not
  change.

`before` and `document` are both full Tiptap JSON documents, so you can diff them to inspect exactly
