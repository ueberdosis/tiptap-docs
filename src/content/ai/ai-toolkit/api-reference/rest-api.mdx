...
<Callout title="Legacy endpoints" variant="info">
  ...
  reference](/ai/ai-toolkit/api-reference/rest-api-v3) for the legacy endpoints.
</Callout>
...
- `format` (`'json' | 'shorthand' | 'shorthandV2'`, optional, default: `'json'`): Document format used by tool
...
- `replaceDocument` (`boolean`, optional)
- `createDocument` (`boolean`, optional): Alias for `replaceDocument`.
- `readSelection` (`boolean`, optional)
...
- `422` with `tool_config_validation_failed`: The tool config provided in the request body is invalid. The response includes `error.data.issues`, a list of validation issues.
...
- `user` (`string`, optional): Author of the edits in version history. It also sets the user associated with AI-generated comments, threads and Tracked Changes suggestions.
  use `reviewOptions.trackedChangesOptions.userId`.
...
- `format` (`'json' | 'shorthand' | 'shorthandV2'`, optional, default: `'json'`): Document format for tool input and
...
- `before` (`JSONContent`): The Tiptap JSON document before the tool was executed.
- `document` (`JSONContent | null`): The Tiptap JSON document after the tool was executed, or `null` when the document did not
  change. Changes may involve modifications in document content and also modifications in the `_hash` attributes of the nodes,
  which are used as unique identifiers and allow the AI to select the nodes of the document.
...
<Callout title="Beta feature" variant="hint">
...
- `document` (`object`, required): Cloud document to stream the edit into:
  ...
  streaming.
- `user` (`string`, optional): Author of the edits in version history, comments and threads (for example with
  `editThreads`). It does not set the author of Tracked Changes suggestions, for Tracked Changes authorship
  use `reviewOptions.trackedChangesOptions.userId`.
- `field` (`string`, optional, default: `"default"`): Collaborative field to target inside a cloud
  document. Streaming supports only the `default` field; any other value is rejected.
...
- `from` (`number`, required): Zero-based index of the first top-level node to read. Must be a
...
- `content` (`JSONContent[] | string`): Document content for the returned range. When `format` is
  `json`, this is a Tiptap JSON fragment. When `format` is `shorthand` or `shorthandV2`, this is a Tiptap Shorthand
  string.
...
- `operations` (`array`, required): One or more edit operations, applied sequentially.
  - `type` (`'replace' | 'insertBefore' | 'insertAfter'`, required): Edit to perform.
  - `target` (`string`, required): Selects a node of the document where the operation will be applied, or the entire document. It is the value of the `_hash` attribute of the selected node, or `"doc"` to target the entire document.
  - `content` (`JSONContent[] | string`, required): Replacement or inserted content. Use a Tiptap
    JSON fragment when `format` is `json`, or a Tiptap Shorthand string when `format` is
    `shorthand` or `shorthandV2`. To delete a node, use an empty fragment.
  - `meta` (`string`, optional): Metadata for the operation. This field is required when the
    `tiptapEdit.meta` option passed to `fetch-tools` is set.
...
- `content` (`JSONContent | string`, required): The replacement document content. Use a Tiptap JSON
  document when `format` is `json`, or a Tiptap Shorthand string when `format` is `shorthand` or
  `shorthandV2`.
...
- `success` (`true`): Indicates that the document was replaced successfully.

Error response:

- `success` (`false`): Indicates that the document could not be replaced.
- `error` (`string`): Error message.

### `createDocument`

Alias for `replaceDocument`. It accepts the same input and returns the same output.

### `readSelection`

Reads the user's current editor selection.

This tool is only compatible with Tiptap Cloud collaborative documents. In [multi-field documents](/editor/extensions/functionality/collaboration#field), only the `default` field is supported.

Tool input (`tool.input`):

This tool does not take any input. Pass an empty object (`{}`).

Tool config (`tool.config`):

- `user` (`string`, required): The ID of the user whose selection to read. Must be the same as the `user.id` defined in the [Collaboration Caret extension](/editor/extensions/functionality/collaboration-caret).

Tool output (`tool.output`):

Success response:

- `success` (`true`): Indicates that the request succeeded.
- `isEmpty` (`false`): A non-empty selection was resolved.
- `content` (`JSONContent[] | string`): The nodes of the document that contain the selection, with special markers delimiting the selection.
- `nodeRange` (`[number, number]`): The node range where the selection is located.

Success response (empty selection):

- `success` (`true`): Indicates that the request succeeded.
- `isEmpty` (`true`): The selection is empty
...
- `from` (`number`, required): Zero-based index of the first thread to read. Must be a non-negative
  integer.
...
- `operations` (`array`, required): One or more thread operations, applied sequentially. Each
  operation has one of these shapes:
  - `createThread`: `type` (`'createThread'`, required), `nodeHash` (`string`, required), and
    `content` (`JSONContent[] | string`, required). `content` is the node content with
    `inlineThread` marks added, as a Tiptap JSON fragment when `format` is `json` or a Tiptap
    Shorthand string when `format` is `shorthand` or `shorthandV2`.
  - `createComment`: `type` (`'createComment'`, required), `threadId` (`string`, required), and
    `content` (`string`, required).
  - `updateComment`: `type` (`'updateComment'`, required), `threadId` (`string`, required),
    `commentId` (`string`, required), and `content` (`string`, required).
  - `removeComment`: `type` (`'removeComment'`, required), `threadId` (`string`, required), and
    `commentId` (`string`, required).
  - `removeThread`: `type` (`'removeThread'`, required) and `threadId` (`string`, required).
  - `resolveThread`: `type` (`'resolveThread'`, required) and `threadId` (`string`, required).
  - `unresolveThread`: `type` (`'unresolveThread'`, required) and `threadId` (`string`, required).
...
- `content` (`JSONContent[] | string`): Whole effective document content. When `format` is `json`,
  this is a Tiptap JSON fragment. When `format` is `shorthand` or `shorthandV2`, this is a Tiptap Shorthand string.

### `proofread`

Applies proofreading edits.

This tool supports `format: "shorthand"` and `format: "shorthandV2"`.

Tool input (`tool.input`):

- `operations` (`array`, required): One or more proofreading operations, applied sequentially.
  - `target` (`string`, required): Selects a node of the document where the operation will be applied, or the entire document. It is the value of the `_hash` attribute of the selected node, or `"doc"` to target the entire document.
  - `delete` (`string`, required): Exact text to remove. Only its first occurrence in the target
    node is replaced.
  - `insert` (`string`, required): Corrected text to insert in place of `delete`.
  - `meta` (`string`, optional): Metadata for the operation. This field is required when the
    `proofread.meta` option passed to `fetch-tools` is set.
