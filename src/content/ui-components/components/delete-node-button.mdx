...
The shortcut is automatically registered when using either the `<DeleteNodeButton />` component or
the `useDeleteNode()` hook. Editor extensions keep ownership of Backspace events they already handled;
the shared delete shortcut runs only for an unhandled event and prevents the browser default only
after a successful deletion. A successful positional deletion starts a separate undo step.
