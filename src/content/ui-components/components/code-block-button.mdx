...
```tsx
import { canToggle } from '@/registry/tiptap-ui/code-block-button'

const canToggle = canToggle(editor) // Check if can toggle
const canTurnInto = canToggle(editor, true) // Check if can turn into code block
```
...
```tsx
import { toggleCodeBlock } from '@/registry/tiptap-ui/code-block-button'
