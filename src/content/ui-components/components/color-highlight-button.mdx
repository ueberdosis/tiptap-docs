...
meta:
  title: Color Highlight Button | Tiptap UI Component
...
You can add the component via Tiptap CLI

```bash
npx @tiptap/cli add color-highlight-button
```
...
### `pickHighlightColorsByValue(values: string[])`

Filters predefined highlight colors (`HIGHLIGHT_COLORS`) by an array of color values. Useful for customizing which highlight options are available in your UI.

```tsx
import { pickHighlightColorsByValue } from '@/registry/tiptap-ui/color-highlight-button'
...
Returns an array of matching highlight color objects.
