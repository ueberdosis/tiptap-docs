...
Tiptap is headless and modular. You control the UI. The **Tiptap UI Components** library gives you prebuilt interfaces so you don’t have to build everything from scratch. Use them as-is or customize to fit your setup.
    ...
    <CardGrid.Wrapper>
        <CardGrid.Item asChild>
            <Link href="/ui-components/templates/simple-editor">
                ...
                <div>
                    <CardGrid.ItemTitle>Use a Full Editor Template</CardGrid.ItemTitle>
                    <CardGrid.ItemParagraph>Get started fast with a full editor that includes commonly used features.</CardGrid.ItemParagraph>
                </div>
                <CardGrid.ItemFooter>
                </CardGrid.ItemFooter>
            </Link>
        ...
        <CardGrid.Item asChild>
            <Link href="/ui-components/components/">
                ...
                <div>
                    <CardGrid.ItemTitle>Use Individual Components</CardGrid.ItemTitle>
...
Some components are built for **Tiptap Cloud features**, like Comments or Version History.

- These components are **not** open source.
- They include frontend logic that depends on Tiptap’s backend services.
...
To build a great editing experience in your Tiptap editor, you can integrate UI elements. The library provides two types of building blocks for this: **(Node) Components** and **Primitives**.
...
These are low-level UI elements like `Button`,`DropdownMenu`,`Popover`,`Spacer`. They’re the foundation of many UI Components, handling accessibility and basic behavior. You can mix and match these primitives to create your own editor controls or to further customize existing components.