...
Use the `compareVersions` utility to compare two Yjs version updates and return a `Changeset` that you can display.
...
`compareVersions` returns a [Changeset](/compare/api-reference/types#changeset). Its `changes` property contains [Change objects](/compare/api-reference/types#change) with the positions where the versions differ, as well as metadata from the version history when available.
...
Display the changeset in the diff editor.
...
Display the changeset with the custom renderer.

```ts
diffEditor.commands.displayChangeset({
  changeset,
