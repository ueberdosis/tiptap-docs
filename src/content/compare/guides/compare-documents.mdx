...
Use the `compareDocuments` utility to compare two Tiptap JSON documents and return a list of changes (a `Changeset`) that you can display.
...
`compareDocuments` returns a [Changeset](/compare/api-reference/types#changeset). Its `changes` property contains the [Change objects](/compare/api-reference/types#change) with the positions where the documents differ.
