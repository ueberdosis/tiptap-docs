...
The result of a document, fragment, step, or version comparison.

```ts
class Changeset {
  /** Original fragment. */
  readonly fragmentA: JSONFragment
  /** Modified fragment. */
  readonly fragmentB: JSONFragment
  /** Changes between both fragments. */
  readonly changes: Change[]

  /** Original fragment as a Tiptap JSON document. */
  get docA(): JSONContent
  /** Modified fragment as a Tiptap JSON document. */
  get docB(): JSONContent

  /** Creates a changeset in the opposite direction,
   *  where document A becomes document B, and vice versa.
   */
  reverse(): Changeset
  /** Serializes the changeset. */
  toJSON(): ChangesetJSON
  /** Hydrates a changeset from serialized JSON. */
  static fromJSON(json: ChangesetJSON): Changeset
}
```
