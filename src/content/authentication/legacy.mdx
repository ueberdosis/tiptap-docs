...
| Content AI | App ID and secret (AI settings) | JWT signed with HS256 |
...
- **Block access.** Set `allowedDocumentNames: []`.
...
You retrieved an App ID and secret from the AI settings page and signed a JWT with HS256. The App ID and token were passed to the AI extensions. REST requests carried the JWT in `Authorization` with the App ID in `X-App-Id`. See [Content AI install](/content-ai/capabilities/generation/install).
