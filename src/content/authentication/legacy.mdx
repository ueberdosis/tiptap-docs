...
| AI Toolkit | App ID and secret (AI settings) | JWT signed with the secret, no extra claims |
...
You retrieved an App ID and secret from the AI settings page and signed a JWT with the secret. The App ID and token were passed to the AI extensions. REST requests carried the JWT in `Authorization` with the App ID in `X-App-Id`. See [AI Toolkit install](/ai/basic/install).
